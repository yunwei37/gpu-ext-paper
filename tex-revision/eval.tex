...
In a separate comparison of scheduling alone and scheduling with memory priority, policies are enabled before CUDA initialization and completion is measured from a common start for both processes.
% 在独立的仅调度与调度加内存优先级比较中，策略在 CUDA 初始化前启用，完成时间从两个进程的共同起点测量。
