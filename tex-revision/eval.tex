...
The tools are \texttt{kernel\-ret\-snoop} (153~LOC) and \texttt{thread\-hist} (89~LOC), whose device-side programs record GPU kernel return timestamps and thread exit-count distributions, respectively, and \texttt{launch\-late} (347~LOC), which combines host- and device-side observations to measure GPU kernel launch latency.
% 工具包括 \texttt{kernel\-ret\-snoop}（153 LOC）和 \texttt{thread\-hist}（89 LOC），其设备端程序分别记录 GPU 内核返回时间戳和线程退出计数分布；\texttt{launch\-late}（347 LOC）结合主机端与设备端观测，测量 GPU 内核启动延迟。
