...
In the KV-cache case, the agent combined stride prefetching for weights with sequential prefetching for KV-cache and adjusted prefetch aggressiveness to PCIe utilization.
% 在 KV-cache 案例中，智能体将权重的步长预取与 KV-cache 的顺序预取相结合，并根据 PCIe 利用率调节预取强度。
For GNN training, it selected one-block lookahead to avoid saturating PCIe.
% 对于 GNN 训练，智能体选择提前一个 block 预取，以避免耗尽 PCIe 带宽。
These findings suggest that programmable memory policies should account for both data access patterns and the bandwidth shared by competing transfers.
% 这些发现表明，可编程内存策略应同时考虑数据访问模式，以及相互竞争的传输所共享的带宽。
\end{minipage}%
}
\par\medskip
...
Combining memory prioritization with scheduling further improves high-priority completion time, demonstrating the benefit of coordinating the two policies.
% 内存优先级与调度相结合，进一步改善高优先级任务的完成时间，说明协调两种策略能够带来额外收益。
...
\caption{Policy comparisons: workload baseline, native policy, and \sys. Baselines are (a) prediction-disabled expert caching, (b) fixed-$K$ FIFO, (c) prefetching all positively predicted experts, (d,g) default scheduling, (e) two-stream FlashAttention, and (f) native CUDA. Panel (e) uses Llama decode batch 128. Bars show medians over five runs per configuration, except (e) means and (f) ten runs. Whiskers in (b,f,g) show full ranges. Panel (g) uses VGG19 LC at 100 requests/s with ResNet152 BE at 100 or 200 requests/s.}
% 策略比较：工作负载基线、原生策略与 gpubpf。基线为关闭预测的专家缓存（a）、固定 K 的 FIFO（b）、预取所有预测会用到的专家（c）、默认调度（d、g）、双流 FlashAttention（e）和原生 CUDA（f）。面板（e）使用 Llama 解码批量 128。柱形为每种配置五次运行的中位数，但（e）为均值、（f）使用十次运行。（b、f、g）的须线表示完整范围。（g）为每秒 100 个 VGG19 LC 请求，对应 ResNet152 BE 每秒 100 或 200 请求。
