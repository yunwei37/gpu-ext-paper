...
Across five paired blocks, the median p99 read latency is 323.707~ms with FIFO, 123.141~ms with the native policy, and 118.097~ms with \sys{}, measured from each request's scheduled arrival.
% 五组配对实验中，读取延迟从各请求的预定到达时刻起算，p99 延迟的中位数在 FIFO、原生策略和 \sys{} 下分别为 323.707、123.141 和 118.097 ms。
...
\caption{Policy comparisons: workload baseline, native policy, and \sys. Baselines are (a) prediction-disabled expert caching, (b) fixed-$K$ FIFO, (c) prefetching all positively predicted experts, (d,g) default scheduling, (e) two-stream FlashAttention, and (f) native CUDA. Panel (e) uses Llama decode batch 128. Bars show medians over five blocks, except (e) means and (f) ten blocks. Whiskers in (b,f,g) show full ranges. Panel (g) uses VGG19 LC at 100 requests/s with ResNet152 BE at 100 or 200 requests/s.}
% 策略比较：工作负载基线、原生策略与 gpubpf。基线为关闭预测的专家缓存（a）、固定 K 的 FIFO（b）、预取所有预测会用到的专家（c）、默认调度（d、g）、双流 FlashAttention（e）和原生 CUDA（f）。面板（e）使用 Llama 解码批量 128。柱形为五组实验的中位数，但（e）为均值、（f）使用十组。（b、f、g）的须线表示完整范围。（g）为每秒 100 个 VGG19 LC 请求，对应 ResNet152 BE 每秒 100 或 200 请求。
...
We then compare the native and \sys{} implementations of the same policy to measure the cost of executing it through \sys{}, keeping the workload and underlying execution unchanged.
% 随后，我们在保持工作负载和底层执行不变的条件下，比较同一策略的原生实现与 \sys{} 实现，以衡量通过 \sys{} 执行该策略的代价。
