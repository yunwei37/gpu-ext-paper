...
\begin{table*}[t]
\centering
\caption{Published policies and their implementation through \sys{}, in the order of \Cref{fig:matched-ports}(a--g).}
% 已发表策略及其通过 \sys{} 实现的方式，顺序对应 \Cref{fig:matched-ports}(a--g)。
...
\toprule
System & Original policy & Execution location and \sys{} implementation \\
% 系统；原有策略；执行位置与 gpubpf 实现。
\midrule
MoE-Infinity~\cite{xue2025moeinfinity} & Match access predictions, rank expert prefetches, and select eviction victims. & Native user-space cache decisions become host-side \sys{} programs; the shared GPT-OSS-120B executor handles expert transfers and inference. \\
% MoE-Infinity：匹配访问预测、排列专家预取顺序并选择驱逐对象；用户态原生缓存决策由主机侧 gpubpf 程序实现，共用的 GPT-OSS-120B 执行器负责专家传输与推理。
\addlinespace
Expert Buffering~\cite{huang2023towards} & Admit experts to the cache and prioritize eviction of experts unused by the current batch. & Native user-space admission and eviction become host-side \sys{} decisions, applied by the shared cache executor. \\
% Expert Buffering：决定专家缓存准入，优先驱逐当前批次不用的专家；用户态原生准入与驱逐决策由主机侧 gpubpf 程序实现，并由共用缓存执行器执行。
\addlinespace
FineMoE~\cite{yu2026finemoe} & Select a dynamic set of experts for speculative prefetch. & Native user-space expert selection becomes a host-side \sys{} program; the shared inference executor performs transfers and computation. \\
% FineMoE：动态选择推测预取的专家集合；用户态原生专家选择由主机侧 gpubpf 程序实现，共用推理执行器完成传输与计算。
\addlinespace
Hummingbird~\cite{hu2026hummingbird} & Admit background work during foreground idle intervals. & Native user-space admission becomes a host-side \sys{} program; the shared DNN executor submits the admitted work. \\
% Hummingbird：在前台空闲区间准入后台工作；用户态原生准入决策由主机侧 gpubpf 程序实现，共用 DNN 执行器提交获准的工作。
\addlinespace
...
\addlinespace
XSched~\cite{shen2025xsched} & Select the highest-priority task using inter-kernel preemption. & Native user-space task selection becomes a host-side \sys{} program; the original executor provides inter-kernel suspend/resume. \\
% XSched：利用内核间抢占选择最高优先级任务；用户态原生任务选择由主机侧 gpubpf 程序实现，原始执行器提供内核间挂起与恢复。
\addlinespace
GPREEMPT~\cite{fan2025gpreempt} & Control task admission and assign timeslices by priority. & Native user-space blocking/release and driver timeslicing become host- and driver-side \sys{} decisions, applied by the original executor and GPU driver. \\
% GPREEMPT：控制任务准入并按优先级分配时间片；用户态原生阻塞与释放决策以及驱动时间片决策，分别由主机侧和驱动侧 gpubpf 程序实现，原始执行器和 GPU 驱动负责执行。
\bottomrule
\end{tabularx}
\end{table*}
...
Under continuous background load, GPREEMPT's policy reduces LC response p99 from 1.796~ms under default scheduling to 1.615~ms with the native policy and 1.610~ms with \sys{} (\Cref{fig:matched-ports}(g)).
% 在连续后台负载下，GPREEMPT 策略将 LC 响应 p99 从默认调度的 1.796 毫秒降至原生策略的 1.615 毫秒和 \sys{} 的 1.610 毫秒（\Cref{fig:matched-ports}(g)）。
