...
Across five paired blocks, median storage-read p99 latency from scheduled arrival is 323.707/123.141/118.097~ms for FIFO/native/\sys{}, respectively.
% 五组配对实验中，从预定到达时刻起算的存储读请求 p99 延迟中位数，FIFO、原生策略和 \sys{} 分别为 323.707、123.141 和 118.097 ms。
...
\addlinespace
GPREEMPT~\cite{fan2025gpreempt} & Control task admission and assign timeslices by priority. & The original system uses user-space blocking/release and a modified GPU driver for timeslice control; \sys{} implements these decisions with host- and driver-side programs. \\
% GPREEMPT：控制任务准入并按优先级分配时间片；原系统使用用户态阻塞与释放，并通过修改后的 GPU 驱动控制时间片，gpubpf 通过主机侧和驱动侧程序实现这些决策。
\bottomrule
\end{tabularx}
\end{table*}
...
\caption{Policy comparisons: workload baseline, native policy, and \sys. Baselines are (a) prediction-disabled expert caching, (b) fixed-$K$ FIFO, (c) prefetching all positively predicted experts, (d,g) default scheduling, (e) two-stream FlashAttention, and (f) native CUDA. Panel (e) uses Llama decode batch 128. Bars show medians over five blocks, except (e) means and (f) ten blocks; whiskers in (b,f,g) show full ranges. Panel (d) uses idle admission; (g) uses VGG19 LC at 100 requests/s with ResNet152 BE at 100 or 200 requests/s.}
% 策略比较：工作负载基线、原生策略与 gpubpf。基线为关闭预测的专家缓存（a）、固定 K 的 FIFO（b）、预取所有预测会用到的专家（c）、默认调度（d、g）、双流 FlashAttention（e）和原生 CUDA（f）。面板（e）使用 Llama 解码批量 128。柱形为五组实验的中位数，但（e）为均值、（f）使用十组；（b、f、g）的须线表示完整范围。（d）使用空闲准入；（g）为每秒 100 个 VGG19 LC 请求，对应 ResNet152 BE 每秒 100 或 200 请求。
\label{fig:matched-ports}
\end{figure*}

\paragraph{Methodology}
% 方法
In \Cref{fig:matched-ports}, the workload baseline versus the native policy measures policy benefit; native versus \sys{} execution of that policy measures mechanism cost, with the workload and underlying execution unchanged.
% 在 \Cref{fig:matched-ports} 中，工作负载基线与原生策略的比较衡量策略收益；在工作负载和底层执行保持不变时，原生与 \sys{} 执行该策略的比较衡量机制代价。
...
On the P40, the tools are \texttt{kernel\-ret\-snoop} (per-block finish timestamps), \texttt{thread\-hist} (load imbalance detection), and \texttt{launch\-late} (GPU kernel launch latency).
% P40 上的工具为 \texttt{kernel\-ret\-snoop}（每块完成时间戳）、\texttt{thread\-hist}（负载不均衡检测）和 \texttt{launch\-late}（GPU 内核启动延迟）。
...
In panel (b), \texttt{kernel\-ret\-snoop} records per-warp kernel returns and \texttt{thread\-hist} counts exits; their overheads are 5.57\% and 2.97\% with \sys{}, versus 99.62\% and 10.35\% with NVBit, while \texttt{launch\-late} incurs 0.22\% versus 8.80\%.
% 在面板（b）中，\texttt{kernel\-ret\-snoop} 记录逐 warp 的内核返回，\texttt{thread\-hist} 统计退出次数；它们在 \sys{} 下的开销为 5.57\% 和 2.97\%，在 NVBit 下为 99.62\% 和 10.35\%，而 \texttt{launch\-late} 的开销为 0.22\% 对 8.80\%。

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{tex-revision/img/results-raw/revision/obs-overhead-with-array.pdf}
\caption{Prefill throughput loss relative to an uninstrumented baseline. (a) P40, Llama 1B. (b) RTX~5090, TinyLlama-1.1B: ten baseline/tool pairs per configuration, except \sys \texttt{kernel\-ret\-snoop} (five independent pairs). Bars show means and whiskers in (b) show run ranges; P40 uses original reported values. For \sys \texttt{kernel\-ret\-snoop}, records are collected after prefill (10.38\,ms, outside prefill timing).}
% 相对未插桩基线的预填充吞吐下降。(a) P40，Llama 1B。(b) RTX~5090，TinyLlama-1.1B：每个配置十组基线/工具配对，gpubpf \texttt{kernel\-ret\-snoop} 使用另外独立测量的五组配对。柱形为均值，（b）中的须线表示运行范围；P40 使用原文报告值。gpubpf \texttt{kernel\-ret\-snoop} 在预填充后收集记录（10.38 ms，不计入预填充计时）。
