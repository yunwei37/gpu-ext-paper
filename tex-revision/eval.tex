...
We additionally compare no policy, memory priority alone, scheduling alone, and their combination, with all policies enabled before CUDA initialization and completion measured from a common start for both processes.
% 我们进一步比较无策略、仅内存优先级策略、仅调度策略以及两者组合，所有策略均在 CUDA 初始化前启用，并从两个进程的共同起点测量完成时间。
Across five runs per configuration, combining memory priority with scheduling reduces high-priority completion time by a median of 10.38\%, 2.31\%, and 1.36\% relative to scheduling alone for HotSpot, GEMM, and K-Means, respectively.
% 每种配置运行五次，相比仅使用调度策略，组合内存优先级与调度策略使 HotSpot、GEMM 和 K-Means 的高优先级任务完成时间分别缩短 10.38\%、2.31\% 和 1.36\%（各次相对改善的中位数）。
...
\includegraphics[width=\textwidth]{tex-revision/img/results-raw/revision/memory-composition.pdf}

\caption{Multi-tenant memory priority differentiation and policy composition. Each panel retains the original comparison (left) and adds a separate comparison of memory and scheduling policies (right). New bars show median completion times over five runs, whiskers show the observed range of total completion time, and diamonds mark high-priority completion. Single 1$\times$ is the original single-process time, and 2$\times$Single 1$\times$ is the theoretical lower bound for the original comparison.}
% 多租户内存优先级区分与策略组合。每个面板保留原有比较（左），并加入独立的内存与调度策略比较（右）。新增条形表示五次运行的完成时间中位数，误差线表示总完成时间的观测范围，菱形标记高优先级任务的完成时间。Single 1× 为原有单进程时间，2×Single 1× 为原有比较的理论下界。
...
\addlinespace
Hummingbird~\cite{hu2026hummingbird} & Split background kernels to fill foreground idle intervals, with priority-aware memory management. & The original system uses CUDA API interception and kernel splitting for execution and modifies UVM for memory management. Our host-side \sys{} program admits background tasks during foreground idle intervals. \\
% Hummingbird：拆分后台 GPU 内核以填充前台空闲区间，并按优先级管理内存。原系统通过 CUDA API 拦截与内核拆分控制执行，并修改 UVM 管理内存，我们的主机侧 gpubpf 程序在前台空闲区间允许后台任务运行。
\addlinespace
...
\addlinespace
XSched~\cite{shen2025xsched} & Select the highest-priority task through preemptible command queues. & The original system uses user-space queue control and, at higher preemption levels, GPU instrumentation or driver interfaces. Our host-side \sys{} program selects tasks with preemption between GPU kernels. \\
% XSched：通过可抢占命令队列选择最高优先级任务。原系统在用户态控制队列，较高抢占级别还使用 GPU 插桩或驱动接口，我们的主机侧 gpubpf 程序选择任务，抢占发生在 GPU 内核之间。
\addlinespace
