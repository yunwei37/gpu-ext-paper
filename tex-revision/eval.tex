...
We compare scheduling alone (Scheduler$^\dagger$) with scheduling and memory priority combined (Combined$^\dagger$) in a separate experiment.
% 我们在独立实验中比较仅调度（Scheduler$^\dagger$）与调度加内存优先级（Combined$^\dagger$）。
Both configurations enable policies before CUDA initialization and measure completion from a common start for the two processes.
% 两种配置均在 CUDA 初始化前启用策略，并从两个进程的共同起点测量完成时间。
...
\caption{Multi-tenant memory priority differentiation (P: Prefetch, E: Evict). Single 1$\times$ and 2$\times$Single 1$\times$ retain the original baseline and theoretical lower bound.}
% 多租户内存优先级区分（P：Prefetch，E：Evict）。Single 1× 和 2×Single 1× 保留原有基线及理论下界。
...
These experiments use driver 575.57.08 with exclusive GPU access and repeated randomized trials.
% 这些实验使用驱动 575.57.08，以独占 GPU 和重复随机试验运行。
Unless stated otherwise, we report medians with confidence intervals. POD's operator plot uses the mean of five per-run means.
% 除非另有说明，我们报告中位数及置信区间。POD 算子图使用五次运行均值的平均值。

\paragraph{Results}
% 结果
MoE-Infinity reduces TTFT by 22.1\% (native) and 22.5\% (\sys{}) relative to prediction-disabled expert caching.
% 在五次配对运行中，MoE-Infinity 相比关闭预测的专家缓存，将 TTFT 降低 22.1\%（原生）和 22.5\%（\sys{}）。
Expert Buffering reduces full expert loads by 12.85\% in both implementations relative to fixed-$K$ FIFO at the same cache capacity (\Cref{fig:matched-ports}(b)).
% 在相同缓存容量下，Expert Buffering 的两种实现相比固定 K 的 FIFO 均将完整专家加载次数减少 12.85\%（\Cref{fig:matched-ports}(b)）。
FineMoE raises \sys{} throughput by 24.6\% over prefetching every positively predicted expert.
% 相比预取所有预测会用到的专家，FineMoE 将 \sys{} 吞吐提高 24.6\%。
Native and \sys{} throughput differ by 0.21\% in paired runs (95\% CI $[-0.17\%,+0.52\%]$).
% 原生与 \sys{} 在配对运行中的吞吐差异为 0.21\%（95\% CI $[-0.17\%,+0.52\%]$）。
Hummingbird reduces LC response p99 by 5.9--9.1\% (\sys{}) and 7.5--8.2\% (native) relative to default scheduling across periodic and BurstGPT arrivals.
% 在周期性和 BurstGPT 到达模式下，Hummingbird 相比默认调度将 LC 响应 p99 降低 5.9--9.1\%（\sys{}）和 7.5--8.2\%（原生）。
With two LC and four BE processes, XSched lowers LC p99 latency from submission to the first block's start from 76.9~s (native CUDA) to 27.0~s (native policy) and 27.3~s (\sys{}) (\Cref{fig:matched-ports}(f)).
% 在两个 LC 和四个 BE 进程下，XSched 将 LC 从提交到首个线程块开始执行的 p99 延迟从 76.9 秒（原生 CUDA）降至 27.0 秒（原生策略）和 27.3 秒（\sys{}）（\Cref{fig:matched-ports}(f)）。
Under continuous background load, GPREEMPT lowers LC response p99 from 1.796~ms (default scheduling) to 1.615~ms (native policy) and 1.610~ms (\sys{}).
% 在连续后台负载下，GPREEMPT 将 LC 响应 p99 从 1.796 毫秒（默认调度）降至 1.615 毫秒（原生策略）和 1.610 毫秒（\sys{}）。
POD-Attention covers ten shapes with five runs each, compared with non-fused FlashAttention and native POD.
% POD-Attention 覆盖十种形状，每种运行五次，与未融合的 FlashAttention 和原生 POD 比较。
At Llama decode batch 128, operator latency falls from 5.360~ms (two-stream FlashAttention) to 4.322~ms (native POD) and 4.344~ms (\sys{}).
% 在 Llama 解码批量 128 下，算子延迟从 5.360 毫秒（双流 FlashAttention）降至 4.322 毫秒（原生 POD）和 4.344 毫秒（\sys{}）。
