...
With two LC and four BE processes, LC p99 latency from submission until the first thread block starts falls from 76.9~s under native CUDA to 27.0~s with the native policy and 27.3~s with \sys{} (\Cref{fig:matched-ports}(f)).
% 在两个 LC 和四个 BE 进程下，LC 从提交到首个线程块开始执行的 p99 延迟 从原生 CUDA 的 76.9 秒降至原生策略的 27.0 秒和 \sys{} 的 27.3 秒（\Cref{fig:matched-ports}(f)）。
...
A host-side \sys{} program implements Expert Buffering's~\cite{huang2023towards} expert admission and eviction policy, prioritizing eviction of experts unused by the current batch; the shared cache executor applies its decisions.
% 主机侧 \sys{} 程序实现 Expert Buffering~\cite{huang2023towards} 专家准入与驱逐策略，优先驱逐当前批次不用的专家；共用的缓存执行器执行这些决策。
...
\caption{Prefill throughput loss relative to an uninstrumented baseline. (a) P40, Llama 1B. (b) RTX~5090, TinyLlama-1.1B: ten baseline/tool pairs per configuration, except \sys kernelretsnoop (five independent pairs). Bars show means and whiskers show RTX~5090 run ranges; P40 uses original reported values. The axis is linear within $\pm1\%$ and logarithmic beyond. For \sys kernelretsnoop, records are collected after prefill (10.38\,ms, outside prefill timing).}
