...
\caption{Multi-tenant memory priority differentiation (P: Prefetch, E: Evict). Single 1$\times$ and 2$\times$Single 1$\times$ retain the original baseline and theoretical lower bound. The last two bars ($\dagger$) compare scheduling alone with the combined policy in separate measurements: medians of five runs, ranges of total completion time, and diamonds for high-priority completion.}
% 多租户内存优先级区分（P：Prefetch，E：Evict）。Single 1× 和 2×Single 1× 保留原有基线及理论下界。最后两根柱子（$\dagger$）通过独立测量比较仅调度与组合策略：展示五次运行的中位数、总完成时间范围，以及用菱形标记的高优先级完成时间。
...
\addlinespace
Hummingbird~\cite{hu2026hummingbird} & Split background kernels to fill foreground idle intervals, with priority-aware memory management. & The original system schedules split kernels on the host, remaps thread-block coordinates on the device, and modifies UVM for memory management. Our host-side \sys{} program admits background tasks during foreground idle intervals. \\
% Hummingbird：拆分后台 GPU 内核以填充前台空闲区间，并按优先级管理内存。原系统在主机侧调度拆分后的 GPU 内核，在设备侧映射线程块坐标，并修改 UVM 管理内存，我们的主机侧 gpubpf 程序在前台空闲区间允许后台任务运行。
\addlinespace
POD-Attention~\cite{kamath2025pod} & Proportionally select prefill or decode work for each SM. & The original system selects work inside the fused GPU kernel. \sys{} implements this selection as a device-side program, retaining the fused attention computation. \\
% POD-Attention：按比例为各 SM 选择 prefill 或 decode 工作。原系统在融合 GPU 内核中选择工作，gpubpf 通过设备侧程序实现这一选择，保留融合注意力计算。
\addlinespace
XSched~\cite{shen2025xsched} & Select the highest-priority task through preemptible command queues. & The original system controls queues on the host, adds device-side kernel-entry checks at Level 2, and uses driver interfaces at Level 3. Our host-side \sys{} program selects tasks with preemption between GPU kernels. \\
% XSched：通过可抢占命令队列选择最高优先级任务。原系统在主机侧控制队列，在 Level 2 加入设备侧内核入口检查，在 Level 3 使用驱动接口，我们的主机侧 gpubpf 程序选择任务，抢占发生在 GPU 内核之间。
\addlinespace
...
\paragraph{Results}
% 结果
With MoE-Infinity's policy, the native implementation and \sys{} reduce TTFT by 22.1\% and 22.5\%, respectively, relative to prediction-disabled expert caching across five paired runs.
% 采用 MoE-Infinity 策略时，五次配对运行中原生实现与 \sys{} 相比关闭预测的专家缓存，分别将 TTFT 降低 22.1\% 和 22.5\%。
For Expert Buffering at the same cache capacity, both the native policy and \sys{} reduce the number of full expert loads by 12.85\% relative to fixed-$K$ FIFO (\Cref{fig:matched-ports}(b)).
...
For POD-Attention, we evaluate ten shapes over five repeated runs against non-fused FlashAttention and native POD implementations.
% 对 POD-Attention，我们对十种形状各重复运行五次，与未融合的 FlashAttention 及原生 POD 实现比较。
