...
We then compare the \sys{} and native (without \sys{}) implementations of the same policy to measure the cost of \sys{}, keeping the workload and underlying execution unchanged.
% 随后，我们在保持工作负载和底层执行不变的条件下，比较同一策略的 \sys{} 实现与原生实现（不使用 \sys{}），以衡量 \sys{} 的执行代价。
...
We measure hook, observability, and map-placement overheads below.
