...
We compare vLLM's CPU-offload mode (\texttt{--cpu-offload-gb 8}) against \sys with UVM policy.
% 我们将 vLLM 的 CPU 卸载模式（\texttt{--cpu-offload-gb 8}）与采用 UVM 的 gpubpf 进行对比。
...
\sys reduces LC P99 launch latency by 96\% (1188$\mu$s $\to$ 53$\mu$s).
% gpubpf 将 LC 的 P99 启动延迟降低 96\%（1188$\mu$秒 $\to$ 53$\mu$秒）。
BE throughput remains nearly unchanged at 11.38 kernels/s with the native scheduler and 11.47 kernels/s with \sys{} (+0.8\%).
% BE 吞吐量基本不变，原生调度器下为 11.38 kernels/s，\sys{} 下为 11.47 kernels/s（+0.8\%）。
...
\Cref{fig:all-kernels-priority} reports high-priority completion speedup relative to Default.
% \Cref{fig:all-kernels-priority} 报告高优先级任务相对 Default 的完成时间加速比。
Both memory policies and scheduling alleviate the slowdown from memory contention, with scheduling yielding larger improvements in these workloads.
% 内存策略和调度都能缓解显存争用带来的性能下降，调度在这些工作负载中带来的改善更大。
...
FineMoE raises \sys{} throughput by 24.6\%, with a 0.21\% difference from native (95\% CI $[-0.17\%,+0.52\%]$).
% FineMoE 将 \sys{} 吞吐提高 24.6\%，与原生实现相差 0.21\%（95\% CI $[-0.17\%,+0.52\%]$）。
...
Across shapes, \sys{} latency ranges from 0.44\% lower to 1.18\% higher than native.
% 在不同形状下，\sys{} 延迟相比原生实现最多降低 0.44\% 或增加 1.18\%。
