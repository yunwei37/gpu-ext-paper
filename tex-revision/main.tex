...
These are policy gains, not an intrinsic BPF speedup; matched policy ports characterize workload-dependent mechanism costs, including 0.7\% overhead for Expert Buffering and mixed storage-latency effects.
% 这些是策略收益而非 BPF 的固有加速；匹配策略移植刻画了随负载变化的机制代价，包括 Expert Buffering 的 0.7\% 开销和有好有坏的存储延迟效应。
