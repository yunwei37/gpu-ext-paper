...
for i, (ax, workload) in enumerate(zip(axes, DATA['workloads'])):
    ...
    ax.set_xticks([0, 1, 2, 3, 4, 5.6, 6.6], labels + ['Scheduler†', 'Combined†'], rotation=90, ha='center')
    ax.set_ylim(bottom=0)
    ax.set_xlim(-.6, 7.2)
    ax.yaxis.set_major_locator(MaxNLocator(3))
    ax.set_axisbelow(True)
    ax.grid(axis='y', alpha=.2)
    ax.set_title(f'({chr(97+i)}) {workload["name"]}', fontsize=7.2, pad=4)
    if i == 0:
...
handles = [Patch(facecolor=colors[0], label='Both running'),
           ...
           Line2D([], [], color='black', marker='D', linestyle='none', markersize=3, label='High priority†'),
           Line2D([], [], color='#2ecc71', linestyle='--', label='Single 1×'),
           Line2D([], [], color='#9b59b6', linestyle=':', label='2×Single 1×')]
fig.legend(handles=handles, loc='upper center', ncol=3, fontsize=7.2, frameon=False, columnspacing=.7, handlelength=1.2, handletextpad=.35, borderaxespad=0)
fig.subplots_adjust(left=.12, right=.99, bottom=.46, top=.70, wspace=.30)
