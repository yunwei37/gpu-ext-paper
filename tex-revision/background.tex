...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth,trim=0 21bp 0 0,clip]{img/gpu_stack_figure.pdf}
...
\begin{figure*}[t]
    ...
    \caption{Page-fault address traces over time across four workloads.}
    % 四个负载随时间变化的缺页地址轨迹。
...
Third, user-space code cannot access privileged driver mechanisms, leaving policies constrained to GPU kernel launch boundaries and unable to execute on latency-critical paths (e.g., page-fault handlers), perform precise preemption on driver or device states, or exploit GPU-side warp-level access patterns.
