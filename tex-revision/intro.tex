...
Agents can now generate complex code and optimize policies in domains such as compiler heuristics~\cite{alphaevolve2025}, datacenter scheduling, LLM serving~\cite{hamadanian2025glia,cheng2025barbarians}, training strategies~\cite{ding2025asap}, and CPU scheduling~\cite{zheng2025schedcp}.
...
Similar to prior work on resource management~\cite{schedext-docs, zussman2025cache_ext, hoiland2018xdp}, \sys{} supports eBPF-based policies.
...
Unlike prior eBPF-based systems (e.g., sched\_ext~\cite{schedext-docs}, cache\_ext~\cite{zussman2025cache_ext}, and XDP~\cite{hoiland2018xdp}), where each callback makes a single synchronous decision at a fixed event, \sys{} allows callbacks that asynchronously transition resources at user-defined events.
...
Policies based on \sys improve throughput by up to $4.8\times$ over framework offloading and $1.76\times$ over application-tuned baselines, and reduce tail latency by up to $2\times$, with low overhead and no application source-code changes or restarts.
% 基于 \sys 的策略相比框架卸载最高提升 $4.8\times$ 吞吐量，相比应用调优的基线最高提升 $1.76\times$，并将尾延迟最多降低 $2\times$，同时开销很低，且无需修改应用源码或重启应用。
Across seven prior policies, the main metrics in \Cref{fig:matched-ports} differ by less than 2\% between \sys{} and native implementations.
% 对于七种已有策略，\Cref{fig:matched-ports} 中各项指标在 \sys{} 实现与原生实现之间的相对差异均小于 2\%。
...
\begin{itemize}[leftmargin=*,itemsep=0pt]
  ...
  \item an evaluation across four GPU workloads, where \sys{} policies improve throughput by up to $4.8\times$ and reduce tail latency by up to $2\times$, differ in performance from prior work policies by less than 2\%, with all policies generated by AI agents and no OS kernel panics.
  % 在四类 GPU 负载上的评估表明，\sys{} 策略将吞吐量最高提升 $4.8\times$，尾延迟最多降低 $2\times$，全部 59 个策略由 AI 智能体生成，且无 OS 内核恐慌。
\end{itemize}
