...
\subsection{RQ1: Safe Agent-Driven Policy Expression and Performance}
% RQ1：安全的智能体驱动策略表达与性能

We first show that \sys{} policies improve oversubscribed workloads in microbenchmarks (\S\ref{sec:rq1-micro}), then that AI agents can safely and productively generate such policies on realistic workloads (\S\ref{sec:rq1-agent}), extend to multi-tenant settings (\S\ref{sec:rq1-mt}), and finally that the same interface re-expresses the policies of published state-of-the-art systems (\S\ref{sec:policy-mechanism}).
% 我们首先展示 \sys{} 策略在微基准上改善超订负载（\S\ref{sec:rq1-micro}），然后展示 AI 智能体能在真实负载上安全高效地生成此类策略（\S\ref{sec:rq1-agent}），并扩展到多租户场景（\S\ref{sec:rq1-mt}），最后展示同一接口能够重新表达已发表最先进系统的策略（\S\ref{sec:policy-mechanism}）。
...
\subsection{RQ2: Mechanism Cost and Overheads}
% RQ2：机制代价与开销
