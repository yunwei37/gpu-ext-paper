...
% 设备端开销。
We evaluate device-side observability tools built on \sys (\Cref{tab:obs-overhead}) using llama.cpp prefill on two GPUs: the P40 (Llama 1B, sequence-mixed) and the RTX~5090 (TinyLlama-1.1B, ten randomized blocks with paired custom NVBit adapters~\cite{villa2019nvbit}).
% 我们在两块 GPU 上使用 llama.cpp 预填充评估基于 gpubpf 构建的设备端可观测工具（\Cref{tab:obs-overhead}）：P40（Llama 1B，序列混合）和 RTX~5090（TinyLlama-1.1B，十个随机区块，配对的定制 NVBit 适配器~\cite{villa2019nvbit}）。
...
An operation-matched measurement on RTX~5090 (16 balanced blocks, 128 fresh processes) finds that direct host mapping costs 9.43$\times$ the device-resident latency for map updates and 1.09$\times$ for lookups, so the placement benefit is largest for write-heavy policies.
% RTX~5090 上一项操作匹配的测量（16 个均衡区块、128 个全新进程）发现，直接主机映射的 map 更新延迟是设备驻留的 9.43 倍，查找为 1.09 倍，因此放置收益对写密集型策略最大。
