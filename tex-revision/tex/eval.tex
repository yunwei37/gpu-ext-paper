...
\Cref{fig:matched-ports} separates two comparisons: a workload baseline versus a native policy measures policy benefit, while native versus \sys{} on the same executor measures port cost; XSched and GPREEMPT appear in \Cref{fig:scheduler-latency}.
% \Cref{fig:matched-ports} 区分两层比较：工作负载基线与原生策略比较策略收益，相同执行器上的原生实现与 \sys{} 比较移植代价；XSched 和 GPREEMPT 见 \Cref{fig:scheduler-latency}。
