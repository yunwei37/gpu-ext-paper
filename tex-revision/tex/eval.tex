...
The historical single-round records in \Cref{fig:all-kernels-priority} show memory-policy completion-time proxies 55--92\% below no-policy execution.
% \Cref{fig:all-kernels-priority} 中的历史单轮记录显示，内存策略的完成时间代理指标比无策略执行低 55--92\%。
...
In a fresh five-block HotSpot comparison, high/low process-duration medians are 56.55/56.60 s without policy, 25.16/26.85 s with memory policy, 3.94/6.08 s with timeslicing, and 3.55/6.46 s with both.
% 在新的五组 HotSpot 对比中，高/低优先级进程耗时中位数为：无策略 56.55/56.60 秒、内存策略 25.16/26.85 秒、时间片策略 3.94/6.08 秒、组合策略 3.55/6.46 秒。
...
\caption{Historical single-round memory-priority records. Bar heights are completion-time proxies, not reconstructed overlap; scheduler engagement was unconfirmed. Single 1$\times$ and 2$\times$Single are single and sequential references, not concurrent lower bounds.}
% 历史单轮内存优先级记录。柱高是完成时间代理指标，而非重建的重叠时间；调度策略是否实际执行未确认。Single 1× 与 2×Single 分别为单进程及顺序参考，而非并发下界。
