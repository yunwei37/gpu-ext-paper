...
% 设备端开销。
We evaluate device-side observability tools built on \sys (\Cref{tab:obs-overhead}) on RTX~5090 using llama.cpp prefill (TinyLlama-1.1B).
% 我们在 RTX~5090 上使用 llama.cpp 预填充（TinyLlama-1.1B）评估基于 gpubpf 构建的设备端可观测工具（\Cref{tab:obs-overhead}）。
Tools include \texttt{kernel\-ret\-snoop} (per-block finish timestamps) and \texttt{thread\-hist} (load imbalance detection), each paired with a custom NVBit adapter~\cite{villa2019nvbit} targeting the same kernel task over ten randomized blocks.
% 工具包括 \texttt{kernel\-ret\-snoop}（每块完成时间戳）和 \texttt{thread\-hist}（负载不均衡检测），各自与针对相同内核任务的定制 NVBit 适配器~\cite{villa2019nvbit} 配对，共十个随机区块。
For the exit-count histogram, \sys's overhead is 4.0\% versus NVBit's 10.3\%.
% 对于退出计数直方图，gpubpf 的开销为 4.0\%，而 NVBit 为 10.3\%。
For the full per-exit record stream, both tools are dominated by record transport (99.66\% vs 99.62\%, NVBit 0.04 points lower).
% 对于完整的逐条退出记录流，两种工具都受记录传输主导（99.66\% 对 99.62\%，NVBit 低 0.04 个百分点）。
The submission reported P40 numbers; this revision measures on RTX~5090, and \texttt{launchlate} is omitted because its host/device cross-clock comparison does not yet meet our precision gate.
% 投稿版报告的是 P40 数字；本修订版在 RTX~5090 上测量，\texttt{launchlate} 因其主机/设备跨时钟对比尚未达到我们的精度门槛而省略。
