...
To exercise policy behavior separately, five further rotated blocks overlap 64 demand reads with 96 background writes of 24~MiB objects, comparing FIFO with identical native/BPF 10~ms write deferral under controlled pressure inputs, not live HBM telemetry.
% 为单独衡量策略行为，另五组轮换实验重叠执行 64 次按需读取与 96 次后台写入，对象大小为 24~MiB；比较 FIFO 与相同的原生/BPF 10~ms 写入延迟策略，使用受控压力输入而非实时 HBM 遥测。
FIFO/native/BPF scheduled-arrival read p99 medians are 298.22/265.30/247.93~ms; each run's nearest-rank p99 is the maximum of its 64 reads, not application TTFT.
% FIFO/原生/BPF 从计划到达时刻计量的读 p99 中位数为 298.22/265.30/247.93~ms；每次运行按最近秩计算的 p99 即 64 次读取中的最大值，不是应用 TTFT。
BPF lowers paired read p99 by a median 12.44\% versus FIFO, but its paired completed-write throughput change has median $-1.67\%$, while read p50 medians rise from 44.88~ms to 181.31/159.26~ms for native/BPF.
% BPF 相对 FIFO 的配对读 p99 降幅中位数为 12.44\%，但完成写入的配对吞吐变化中位数为 $-1.67\%$；读 p50 中位数则从 44.88~ms 增至原生/BPF 的 181.31/159.26~ms。
This is a tail-versus-median and bandwidth tradeoff; BPF/native paired p99 changes have median $+0.54\%$ and range $-15.88\%$ to $+6.32\%$, not a tight mechanism-overhead bound.
% 这是尾延迟与中位延迟、带宽之间的取舍；BPF/原生配对 p99 变化中位数为 $+0.54\%$，范围为 $-15.88\%$ 至 $+6.32\%$，并非紧确的机制开销界限。
...
On the XSched workload, BE throughput stays within 1\% of native (10.2~kernels/s); on the GPreempt workload, the same LC protection costs about 9\% BE goodput (\S\ref{sec:policy-mechanism}).
% 在 XSched 工作负载上，BE 吞吐量保持在原生的 1\% 以内（10.2 内核/秒）；在 GPreempt 工作负载上，同样的 LC 保护付出约 9\% 的 BE 有效吞吐代价（\S\ref{sec:policy-mechanism}）。
...
With differentiated timeslices (LC: 1~s, BE: 200~$\mu$s), the same \sys{} policy reduces LC P99 launch latency by 96\% (1188~$\mu$s $\to$ 53~$\mu$s) while BE throughput remains unchanged (\Cref{fig:scheduler-throughput}).
% 采用差异化时间片（LC：1 秒、BE：200 微秒）时，同一 gpubpf 策略将 LC 的 P99 启动延迟降低 96%（1188 微秒降至 53 微秒），BE 吞吐量保持不变（\Cref{fig:scheduler-throughput}）。

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{img/results-raw/revision/scheduling-comparison-lc-bars.pdf}

\caption{Matched scheduling policies on RTX~5090, LC latency: (a) XSched workload, two LC and four BE processes, LC queue-entry p99; (b) VGG19 LC at 100 requests/s against three ResNet152 BE loads, arrival-to-verified-response p99. Bars show medians (XSched: ten paired repetitions; GPreempt: five per load); background throughput and goodput are in \S\ref{sec:policy-mechanism}.}
% RTX 5090 上的匹配调度策略，LC 延迟：(a) XSched 工作负载，两个 LC 和四个 BE 进程，LC 入队 p99；(b) VGG19 LC 每秒 100 请求、对应三种 ResNet152 BE 负载，到已验证响应的 p99。柱形为中位数（XSched：十次配对重复；GPreempt：每种负载五次）；后台吞吐与有效吞吐见 \S\ref{sec:policy-mechanism}。
\label{fig:scheduler-latency}
\label{fig:matched-scheduling}
\end{figure}
% --- Earlier full-width versions of this figure retained: img/results-raw/revision/scheduling-comparison-1x4-bars.pdf and scheduling-comparison-2x2.pdf ---
...
\label{fig:scheduler-throughput}
\end{figure}
...
% --- Merged overhead table (replaced by the bar figure below; retained for reference) ---
% \begin{table}[t]
% \centering
% \small
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{img/results-raw/revision/obs-overhead-bars.pdf}
\caption{\sys device-side observability tools: llama.cpp prefill-throughput overhead versus matched NVBit instrumentation (lower is better, log scale). (a) P40 with Llama 1B, sequence-mixed. (b) RTX~5090 with TinyLlama-1.1B and custom paired NVBit adapters; bars show the mean of ten paired runs per tool.}
% gpubpf 设备端可观测工具：与配对 NVBit 插桩相比的 llama.cpp 预填充吞吐开销（越低越好，对数坐标）。(a) P40，Llama 1B，序列混合。(b) RTX~5090，TinyLlama-1.1B，配定制配对 NVBit 适配器；柱形为每个工具十次配对运行的均值。
