...
Under continuous background load, this policy reduces LC response p99 from 1.796~ms under default scheduling to 1.615~ms with the original C policy and 1.610~ms with \sys{} (\Cref{fig:matched-ports}(g)).
% 在连续后台负载下，GPREEMPT~\cite{fan2025gpreempt} 的优先级时间片将 LC 响应 p99 从默认调度的 1.796 毫秒降至原始 C 策略的 1.615 毫秒和 \sys{} 的 1.610 毫秒（\Cref{fig:matched-ports}(g)）。
...
On the RTX~5090, \sys has lower overhead than NVBit for all three tools: 90.71\% versus 99.62\% for the complete per-warp kernel-return stream, 2.97\% versus 10.35\% for the exit-count histogram, and 0.22\% versus 8.80\% for \texttt{launchlate}.
% 在 RTX~5090 上，gpubpf 在三个工具上的开销均低于 NVBit：完整的逐 warp 内核返回记录流为 90.71\% 对 99.62\%，退出计数直方图为 2.97\% 对 10.35\%，\texttt{launchlate} 为 0.22\% 对 8.80\%。
For 512-token prefill on the RTX~5090, buffering the complete per-warp return records in GPU memory reduces prefill overhead to 5.57\% across five additional baseline/tool pairs, with records collected after prefill.
% 在 RTX~5090 的 512-token 预填充中，将完整的逐 warp 返回记录缓存在 GPU 内存中，并在预填充后收集，使另外五组基线/工具配对测量的预填充开销降至 5.57\%。
...
\caption{Prefill throughput loss relative to an uninstrumented baseline. (a) P40, Llama 1B. (b) RTX~5090, TinyLlama-1.1B: ten pairs per tool, plus five independent pairs for \sys GPU buffering. Bars show means, with whiskers for the RTX~5090 run ranges; P40 shows original reported values. The axis is linear within $\pm1\%$ and logarithmic beyond. GPU buffering yields 5.57\% prefill overhead; its final collection (10.38\,ms) occurs outside prefill timing.}
% 相对未插桩基线的预填充吞吐下降。(a) P40，Llama 1B。(b) RTX~5090，TinyLlama-1.1B：每个工具十组配对，另加 gpubpf GPU 缓冲的五组独立配对。柱形表示均值，须线表示 RTX~5090 各次运行的范围；P40 使用原文报告值。坐标轴在 ±1\% 内为线性，之外为对数。预填充计时不含最终收集（GPU 缓冲为 10.38 ms）；GPU 缓冲的预填充开销为 5.57\%。
