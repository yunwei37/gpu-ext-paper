...
\Cref{fig:matched-ports} separates two comparisons: a workload baseline versus a native policy implementation measures policy benefit, while native versus \sys{} on the same executor measures the port's cost; XSched and GPREEMPT appear in \Cref{fig:matched-scheduling}.
% \Cref{fig:matched-ports} 区分两层比较：工作负载基线与原生策略实现比较策略收益，相同执行器上的原生实现与 \sys{} 比较移植代价；XSched 和 GPREEMPT 见 \Cref{fig:matched-scheduling}。
...
\paragraph{Compute-kernel scheduling.}
% 计算内核调度。
The original compute-kernel workload runs two LC and four BE processes, each submitting 50 kernels through four CUDA streams; \Cref{fig:scheduler-latency} reports launch latency and BE throughput with differentiated timeslices.
% 原有计算内核工作负载运行两个 LC 和四个 BE 进程，每个进程通过四条 CUDA 流提交 50 个内核；\Cref{fig:scheduler-latency} 报告差异化时间片下的启动延迟和 BE 吞吐量。
