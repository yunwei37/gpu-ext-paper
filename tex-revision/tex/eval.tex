...
On the RTX~5090, which retains the complete per-exit record stream (23~M records), \sys's exit-count histogram overhead is 4.0\% versus NVBit's 10.3\%; the full per-exit record stream is dominated by record transport for both tools (99.66\% vs 99.62\%).
% 在保留完整逐条退出记录流（2,300 万条记录）的 RTX~5090 上，gpubpf 的退出计数直方图开销为 4.0\%，而 NVBit 为 10.3\%；完整的逐条退出记录流对两种工具都受记录传输主导（99.66\% 对 99.62\%）。
