...
We answer RQ1 in \S\ref{sec:rq1} through policy microbenchmarks, agent-driven case studies on realistic workloads, multi-tenant settings, and matched ports of published policies. \S\ref{sec:rq2-cost} answers RQ2.
% 我们在 \S\ref{sec:rq1} 通过策略微基准、真实负载上的智能体驱动案例研究、多租户场景，以及对已发表策略的匹配移植回答 RQ1；\S\ref{sec:rq2-cost} 回答 RQ2。
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{img/results-raw/revision/matched-policy-panels.pdf}
\caption{Matched policy ports on RTX~5090: workload baseline, native policy, and \sys on the same executor. Baselines are (a) prediction-disabled expert caching, (b) fixed-$K$ FIFO, (c) all-positive prefetch, (d,g) default scheduling, (e) two-stream FlashAttention, and (f) native CUDA. TTFT measures first-visible-text latency; panel (e) uses Llama decode batch 128. Bars show medians over five blocks, except (e) means and (f) ten blocks; whiskers in (b,f,g) show full ranges. Panel (d) uses idle admission; (g) uses VGG19 LC at 100 requests/s with ResNet152 BE at 100 or 200 requests/s, or continuous load.}
% RTX~5090 上的匹配策略移植：相同执行器上的工作负载基线、原生策略与 gpubpf。基线为关闭预测的专家缓存（a）、固定 K 的 FIFO（b）、全正例预取（c）、默认调度（d、g）、双流 FlashAttention（e）和原生 CUDA（f）。TTFT 表示首个可见文本延迟；面板（e）使用 Llama 解码批量 128。柱形为五组实验的中位数，但（e）为均值、（f）使用十组；（b、f、g）的须线表示完整范围。（d）使用空闲准入；（g）为每秒 100 个 VGG19 LC 请求，对应 ResNet152 BE 每秒 100、200 请求或连续负载。
...
% MoE-Infinity（专家卸载）
Host-side \sys{} programs implement MoE-Infinity's~\cite{xue2025moeinfinity} prediction matching, prefetch ranking, and eviction selection, while the shared GPT-OSS-120B executor performs expert transfers and inference.
% 主机侧 \sys{} 程序实现 MoE-Infinity~\cite{xue2025moeinfinity} 的预测匹配、预取排序和驱逐选择，共用的 GPT-OSS-120B 执行器负责专家传输与推理。
...
% XSched（抢占式调度）
We port XSched's~\cite{shen2025xsched} inter-kernel preemption: a host-side \sys{} program selects the highest-priority task, and the original executor performs suspend/resume.
% 我们移植 XSched~\cite{shen2025xsched} 的内核间抢占：主机侧 \sys{} 程序选择最高优先级任务，原始执行器执行挂起/恢复。
With two latency-critical (LC) and four best-effort (BE) processes, LC submission-to-first-CTA p99 falls from 76.9~s under native CUDA to 27.0~s with the native policy and 27.3~s with \sys{} (\Cref{fig:matched-ports}(f)).
% 在两个延迟关键（LC）和四个尽力型（BE）进程下，LC 提交到首个 CTA 的 p99 从原生 CUDA 的 76.9 秒降至原生策略的 27.0 秒和 \sys{} 的 27.3 秒（\Cref{fig:matched-ports}(f)）。

\paragraph{GPREEMPT (priority timeslicing)}
% GPREEMPT（优先级时间片）
For GPREEMPT~\cite{fan2025gpreempt}, host-side \sys{} programs issue blocking and release decisions, and driver-side \sys{} programs assign priority-based timeslices; the original executor and GPU driver carry out these decisions.
% 对 GPREEMPT~\cite{fan2025gpreempt}，主机侧 \sys{} 程序做出阻塞与释放决策，驱动侧 \sys{} 程序按优先级分配时间片，由原始执行器和 GPU 驱动执行。
Under continuous background load, this policy reduces LC response p99 from 1.796~ms under default scheduling to 1.615~ms with the native policy and 1.610~ms with \sys{} (\Cref{fig:matched-ports}(g)).
% 在连续后台负载下，GPREEMPT~\cite{fan2025gpreempt} 的优先级时间片将 LC 响应 p99 从默认调度的 1.796 毫秒降至原生策略的 1.615 毫秒和 \sys{} 的 1.610 毫秒（\Cref{fig:matched-ports}(g)）。

\paragraph{Expert Buffering (expert residency)}
% Expert Buffering（专家驻留）
A host-side \sys{} program implements Expert Buffering's~\cite{huang2023towards} inactive-first expert admission and eviction decisions, which the shared cache executor applies.
% 主机侧 \sys{} 程序实现 Expert Buffering~\cite{huang2023towards} 非活跃优先的专家准入与驱逐决策，由共用的缓存执行器执行。
At the same cache capacity, both the native policy and \sys{} reduce whole-expert loads by 12.85\% relative to fixed-$K$ FIFO (\Cref{fig:matched-ports}(b)).
% 在相同缓存容量下，原生策略与 gpubpf 均相对固定 K 的 FIFO 将完整专家加载次数减少 12.85\%（\Cref{fig:matched-ports}(b)）。

\paragraph{FineMoE (dynamic expert sets)}
% FineMoE（动态专家集）
A host-side \sys{} program implements FineMoE's~\cite{yu2026finemoe} dynamic-set rule to select experts for speculative prefetch, while the shared inference executor performs their transfers and computation.
% 主机侧 \sys{} 程序实现 FineMoE~\cite{yu2026finemoe} 的动态集合规则，选择推测预取的专家，由共用的推理执行器完成传输与计算。
...
% Hummingbird（空闲准入）
A host-side \sys{} program implements Hummingbird's~\cite{hu2026hummingbird} idle-admission rule by deciding when background work may enter a foreground idle interval, while the shared DNN executor submits the admitted work.
% 主机侧 \sys{} 程序实现 Hummingbird~\cite{hu2026hummingbird} 的空闲准入规则，决定后台工作何时可进入前台空闲区间，由共用的 DNN 执行器提交获准的工作。
Across periodic and BurstGPT arrivals, \sys{} lowers LC response p99 by 5.9--9.1\% relative to default scheduling, compared with 7.5--8.2\% for the native policy.
% 在周期性和 BurstGPT 到达模式下，相对默认调度，\sys{} 将 LC 响应 p99 降低 5.9--9.1\%，原生策略降低 7.5--8.2\%。
...
% 机制代价
Expert Buffering incurs 0.7\% overhead; POD's paired operator-latency effects range from a 0.44\% reduction to a 1.18\% increase across ten shapes; a kernel-verified UVM no-prefetch policy incurs 3.2\% overhead over fifteen paired fault-path runs.
% Expert Buffering 的开销为 0.7\%；POD 在十种形状上的配对算子延迟效应从降低 0.44\% 到增加 1.18\%；经内核验证的 UVM 无预取策略在十五次配对缺页路径运行中的开销为 3.2\%。
...
\caption{Prefill throughput loss relative to an uninstrumented baseline. (a) P40, Llama 1B. (b) RTX~5090, TinyLlama-1.1B: ten baseline/tool pairs per arm, except \sys kernelretsnoop, which uses five independently measured pairs. Bars show means and whiskers show RTX~5090 run ranges; P40 uses original reported values. The axis is linear within $\pm1\%$ and logarithmic beyond. For \sys kernelretsnoop, records are collected after prefill (10.38\,ms, outside prefill timing).}
% 相对未插桩基线的预填充吞吐下降。(a) P40，Llama 1B。(b) RTX~5090，TinyLlama-1.1B：每个配置十组基线/工具配对，gpubpf kernelretsnoop 使用另外独立测量的五组配对。柱形为均值，须线为 RTX~5090 的运行范围；P40 使用原文报告值。坐标轴在 ±1\% 内为线性，之外为对数。gpubpf kernelretsnoop 在预填充后收集记录（10.38 ms，不计入预填充计时）。
