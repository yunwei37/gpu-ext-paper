...
Default inputs make all storage decisions submit immediately: the paired BPF/native throughput difference has median $+0.28\%$ (range $-3.42\%$ to $+5.17\%$), measuring admission cost.
% 默认输入使存储决策均为立即提交：BPF/原生配对吞吐差异中位数为 $+0.28\%$（范围 $-3.42\%$ 至 $+5.17\%$），衡量准入代价。
LMCache owns transfers and buffers; cuFile uses compatibility mode here.
% LMCache 管理传输和缓冲区；cuFile 在此使用兼容模式。
