...
\caption{Matched scheduling policies on RTX 5090. Left: XSched workload, two LC and four BE processes; (a) LC queue-entry p99; (c) BE kernels/s. Right: VGG19 LC at 100 requests/s against three ResNet152 BE loads; (b) arrival-to-verified-response p99; (d) in-window BE goodput. Points show individual cells; horizontal bars show medians.}
% RTX 5090 上的匹配调度策略。左：XSched 工作负载，两个 LC 和四个 BE 进程；(a) LC 入队 p99；(c) BE 内核数/秒。右：VGG19 LC 每秒 100 请求，对应三种 ResNet152 BE 负载；(b) 到已验证响应的 p99；(d) 窗口内 BE 有效吞吐。点为各个单元，横线为中位数。
