...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{RQ5 (Policy versus Mechanism):} Can \sys{} express the policies of published state-of-the-art systems, and what does the mechanism cost when running the same policy?
  % RQ5（策略与机制）：gpubpf 能否表达已发表的最先进系统的策略？用该机制运行相同策略的代价是多少？
\end{itemize}
...
RQ5 additionally compares against published research systems: MoE-Infinity~\cite{xue2025moeinfinity}, XSched~\cite{shen2025xsched}, GPREEMPT~\cite{fan2025gpreempt}, Expert Buffering~\cite{huang2023towards}, FineMoE~\cite{yu2026finemoe}, Hummingbird~\cite{hu2026hummingbird}, and POD-Attention~\cite{kamath2025pod}, each evaluated as a matched pair of the original implementation and a \sys{} port of the same policy (\S\ref{sec:policy-mechanism}).
% RQ5 进一步与已发表的研究系统对比：MoE-Infinity、XSched、GPREEMPT、Expert Buffering、FineMoE、Hummingbird 和 POD-Attention，每一组都是原始实现与同一策略的 gpubpf 移植版的匹配对比（\S\ref{sec:policy-mechanism}）。
...
On the RTX~5090, which retains the complete per-exit record stream (23~M records), \sys's exit-count histogram overhead is 4.0\% versus NVBit's 10.3\%; the full per-exit record stream is dominated by record transport for both tools (99.66\% vs 99.62\%), and \texttt{launchlate} adds 30.6\% versus NVBit's 18.0\%.
% 在保留完整逐条退出记录流（2,300 万条记录）的 RTX~5090 上，gpubpf 的退出计数直方图开销为 4.0\%，而 NVBit 为 10.3\%；完整的逐条退出记录流对两种工具都受记录传输主导（99.66\% 对 99.62\%），\texttt{launchlate} 的开销为 30.6\%，NVBit 为 18.0\%。
...
kernelretsnoop & RTX~5090 & 99.663\% & 99.621\% \\
threadhist & RTX~5090 & 4.007\% & 10.301\% \\
launchlate & RTX~5090 & 30.6\% & 18.0\% \\
\bottomrule
\end{tabularx}
