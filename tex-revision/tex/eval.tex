...
Unless otherwise specified, the original experiments run on Server~A, averaging 10 trials using geometric mean; the added revision studies report their own repetitions and summary statistics.
% 除非另有说明，原始实验均在服务器 A 上进行，每项结果取 10 次试验的几何平均值；新增修订研究分别报告其重复次数和汇总统计量。
...
The original experiments use PyTorch (version 2.9.0) and vLLM (version 0.11.0) with custom allocators, llama.cpp (version 7101), and Faiss (version 1.13.0) with UVM build config; revision artifacts record their separate software configurations.
% 原始实验使用 PyTorch（版本 2.9.0）和 vLLM（版本 0.11.0，配自定义分配器）、llama.cpp（版本 7101）以及采用 UVM 构建配置的 Faiss（版本 1.13.0）；修订工件另行记录各自的软件配置。
...
In the original agent study, we provided an AI coding agent (Claude Code with Opus 4.5) with \sys{}'s eBPF interfaces; the agent autonomously generated all policy logic in that study without human review or guidance, while humans authored only the initial framework and benchmark harness.
% 在原始智能体研究中，我们向一个 AI 编程智能体（Claude Code，Opus 4.5）提供 gpubpf 的 eBPF 接口；智能体在无人审阅或指导的情况下自主生成了该研究中的全部策略逻辑，人类只编写初始框架和基准测试工具。
All 59 policies in that study were produced via a fully automated generate-test-iterate loop; the added research-policy ports and storage-tier study are separate, coordinated revision work, not part of that 59-policy count or no-human-guidance claim.
% 该研究的全部 59 个策略均由完全自动化的“生成—测试—迭代”循环产出；新增的研究策略移植和存储层研究属于另行协调的修订工作，不计入这 59 个策略，也不属于无人指导的声明范围。
...
These hints improve decode over default UVM but require application modification and degrade prefill by 40\%; \sys keeps prefill within 4\% of default UVM, without application changes.
% 这些提示相比默认 UVM 改善了解码，但需要修改应用，并使预填充性能下降 40\%；gpubpf 将预填充保持在默认 UVM 的 4\% 以内，且无需修改应用。
For prefill (compute-bound), framework offloading (\texttt{ncmoe=32}) achieves 13\% higher prefill throughput than \sys{}; this prefill/decode tradeoff pairs the prefill gap against the larger decode gains above.
% 对于预填充（计算受限），框架卸载（\texttt{ncmoe=32}）比 gpubpf 的预填充吞吐量高 13\%；这一预填充/解码取舍以预填充差距对应上文更大的解码收益。
...
In the framework configurations we evaluate, CPU offloading targets either experts or KV-cache, not both; UVM attempts on-demand migration of both, leading to thrashing.
% 在我们评估的框架配置中，CPU 卸载只针对专家权重或 KV-cache 之一，而非二者兼有；UVM 则尝试对二者按需迁移，导致抖动。
...
UVM without \sys policies performs worse than vLLM's offload, while \sys matches the best framework offloading with better tail latency, dynamically and transparently.
% 没有 gpubpf 策略的 UVM 表现不如 vLLM 的卸载，而 gpubpf 以动态、透明的方式与最佳框架卸载持平，且尾延迟更优。
...
UVM with \sys prefetching enables 15M-node training at 2.17$\times$ oversubscription without application modification.
% 采用 gpubpf 预取的 UVM 使 15M 节点（2.17× 超订）训练得以进行，且无需修改应用。
...
Suspecting lock contention initially, the agent identified PCIe bandwidth saturation as the true bottleneck, making one-block lookahead the best of the tested settings.
% 智能体最初怀疑锁竞争，随后确认真正的瓶颈是 PCIe 带宽饱和，单块前瞻为所测设置中的最佳。
...
We evaluate whether \sys{} can express published state-of-the-art policies, and what the general mechanism costs when running the same policy as an ad-hoc implementation.
% 我们评估 \sys{} 能否表达已发表的最先进策略，以及通用机制运行与专用实现相同的策略时的代价。
\Cref{fig:matched-ports} separates two comparisons: a workload baseline versus a native policy shows deployment benefit or cost, while native versus \sys{} on the same executor measures port cost; XSched and GPREEMPT appear in \Cref{fig:scheduler-latency}.
% \Cref{fig:matched-ports} 区分两层比较：工作负载基线与原生策略比较部署的收益或代价，相同执行器上的原生实现与 \sys{} 比较移植代价；XSched 和 GPREEMPT 见 \Cref{fig:scheduler-latency}。

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{img/results-raw/revision/matched-port-panels.pdf}
\caption{Matched policy ports on RTX~5090. Baselines are (a) prediction-disabled expert caching with a prefill overload shortcut, (b) fixed-$K$ FIFO, (c) all-positive prefetch, (d) default scheduling, and (e) two-stream FlashAttention. Each panel reports one metric over five blocks: secondary first-visible-text latency median (a), median throughput (b,c), median LC response p99 (d), and mean operator latency for Llama decode batch 128 (e). Native-policy bars use the same executor as \sys{}; panel (d) uses the idle-admission component.}
% RTX~5090 上的匹配策略移植。基线依次为关闭预测且保留预填充超额加载捷径的专家缓存、固定 K 的 FIFO、全正例预取、默认调度和双流 FlashAttention。每个面板报告五组实验中的一个指标：次要指标首个可见文本延迟中位数（a）、吞吐中位数（b、c）、LC 响应 p99 中位数（d），以及 Llama 解码批量 128 的平均算子延迟（e）。原生策略与 \sys{} 使用相同执行器；面板（d）使用空闲准入组件。
\label{fig:matched-ports}
\end{figure*}

\paragraph{Methodology}
% 方法
All comparisons use matched workloads on RTX~5090 (driver 575.57.08) with exclusive GPU access and repeated randomized trials.
% 所有对比使用匹配的工作负载，在 RTX~5090（驱动 575.57.08）上以独占 GPU 和重复随机试验运行。
...
Across five paired runs, native and \sys{} reduce first-visible-text latency by 22.1\% and 22.5\%, respectively, over prediction-disabled expert caching.
% 五次配对试验中，相对关闭预测的专家缓存，原生与 \sys{} 分别将首个可见文本延迟降低 22.1\% 和 22.5\%。
However, primary throughput medians (including final prefetch drain) are 11.90/11.22/11.19 token/s for baseline/native/\sys{}; lower secondary visible-text latency does not reverse this throughput loss.
% 但包含最终预取排空的主要吞吐中位数为基线/原生/\sys{} 的 11.90/11.22/11.19 token/s；更低的次要首段可见文本延迟并未改变这一吞吐损失。
The paired \sys{}/native throughput change is $-0.35\%$ (95\% CI $[-1.08\%,+0.55\%]$).
% \sys{} 相对原生的配对吞吐变化为 $-0.35\%$（95\% CI $[-1.08\%,+0.55\%]$）。
Only the baseline retains a prefill extra-expert overload shortcut, so baseline/policy contrasts also change executor behavior and do not isolate policy benefit.
% 仅基线保留预填充时额外加载专家的超额捷径，因此基线与策略的对比还改变了执行器行为，不能单独归因于策略收益。
% Evidence: workloads/moe-infinity/results-paper-v3-protected-575.md; five completed blocks, no rerun.
...
BE throughput medians are 10.2377/10.1497/10.1616 kernels/s for native CUDA, XSched, and \sys{} respectively.
% 原生 CUDA、XSched 与 \sys{} 的 BE 吞吐中位数分别为 10.2377/10.1497/10.1616 kernels/s。
% Evidence: workloads/xsched/performance-full-575-20260903.md; 46/46 cells accepted; no rerun.

\paragraph{GPREEMPT (priority timeslicing)}
% GPREEMPT（优先级时间片）
For GPREEMPT~\cite{fan2025gpreempt}, host-JIT \sys{} programs issue blocking and release decisions and kernel-BPF programs assign priority-based timeslices; the original executor and GPU driver carry them out.
...
BE goodput under the same continuous background load falls from 197.717~req/s for the native baseline to 179.967 with the original C policy and 180.100 with \sys{}, so both ports trade background throughput for foreground protection.
% 相同连续后台负载下，BE 良吞吐从原生基线的 197.717 req/s 降至原始 C 策略的 179.967 与 \sys{} 的 180.100 req/s，两个移植以前台保护换取后台吞吐。
% Evidence: workloads/gpreempt/results-load-study-575-20260903.md; all 45 cells accepted; no rerun.
...
A host-JIT \sys{} program implements Expert Buffering's~\cite{huang2023towards} inactive-first expert admission and eviction; the shared cache executor applies them.
% 主机 JIT 的 \sys{} 程序实现 Expert Buffering~\cite{huang2023towards} 非活跃优先的专家准入与驱逐；共用的缓存执行器执行这些决策。
...
% FineMoE（动态专家集）
A host-JIT \sys{} program implements FineMoE's~\cite{yu2026finemoe} dynamic-set rule for speculative-prefetch expert selection; the shared inference executor performs the transfers and computation.
% 主机 JIT 的 \sys{} 程序实现 FineMoE~\cite{yu2026finemoe} 用于推测预取专家选择的动态集合规则；共用的推理执行器完成传输与计算。
\sys{} improves throughput by 24.6\% over all-positive prefetch (fetching every positive prediction); native and \sys{} throughput differ by 0.21\% in paired runs (95\% CI $[-0.17\%,+0.52\%]$).
% 相比预取所有正预测的全正例预取，\sys{} 将吞吐提高 24.6\%；原生与 \sys{} 的配对吞吐差异为 0.21\%（95\% CI $[-0.17\%,+0.52\%]$）。
...
% Hummingbird（空闲准入）
A host-JIT \sys{} program implements Hummingbird's~\cite{hu2026hummingbird} idle-admission rule, deciding when background work may enter a foreground idle interval; the shared DNN executor submits the admitted work.
% 主机 JIT 的 \sys{} 程序实现 Hummingbird~\cite{hu2026hummingbird} 的空闲准入规则，决定后台工作何时可进入前台空闲区间；共用的 DNN 执行器提交获准的工作。
...
Both ports pay for this foreground protection with background throughput: BE goodput medians fall from 179.2333/179.1500 req/s under native CUDA admission to 133.2167/133.1667 req/s with the C port and 133.0500/132.8167 req/s with the BPF port.
% 两个移植以前台保护换取后台吞吐：BE 良吞吐中位数从原生 CUDA 准入的 179.2333/179.1500 req/s 降至 C 移植的 133.2167/133.1667 req/s 与 BPF 移植的 133.0500/132.8167 req/s。
...
% 机制代价
Execution costs are operation-specific examples from particular workloads and hooks, not a universal overhead bound: Expert Buffering incurs 0.7\% overhead; POD's paired operator-latency effects range from a 0.44\% reduction to a 1.18\% increase across ten shapes; a kernel-verified UVM no-prefetch policy adds 3.2\% overhead over fifteen paired fault-path runs.
% 执行代价是特定工作负载与钩子下的操作级示例，而非通用的开销界限：Expert Buffering 的开销为 0.7\%；POD 在十种形状上的配对算子延迟效应从降低 0.44\% 到增加 1.18\%；经内核验证的 UVM 无预取策略在十五次配对缺页路径运行中的开销为 3.2\%。
Storage measurements also limit this attribution: BPF/native paired p99 differences in the LMCache port (\S\ref{sec:rq1-agent}) range from $-39.7\%$ to $+33.8\%$ across five blocks of one workload, so the variability precludes a tight estimate of mechanism overhead.
% 存储测量也限制了这种归因：LMCache 移植（\S\ref{sec:rq1-agent}）同一工作负载五个组内，BPF 相对原生的配对 p99 差异从 -39.7\% 到 +33.8\%，该变动使得机制开销无法得到精确估计。

\paragraph{Agent-discovered policies}
% 智能体发现的策略
Beyond reproducing published policies, we also evaluate agent-discovered combinations of cross-layer observations and controls.
% 除了复现已发表的策略，我们还评估了智能体发现的跨层观测与控制组合。
In the MoE study (\S\ref{sec:eval}), device-side tracing led the agent to a stride-prefetch policy aligned with per-layer expert weight layout combined with LFU eviction that protects shared layers; in the KV-cache study, the agent differentiated prefetch by region type (stride for weights, sequential for KV-cache) under a PCIe-utilization budget, avoiding the mutual thrashing caused by a uniform policy.
...
Both combinations couple device-side observation with host-side action and share the tradeoff that migration spends transfer bandwidth that demand traffic also needs.
% 两种组合都将设备端观测与主机端动作耦合，并共有一个权衡：迁移会占用需求流量所需的传输带宽。
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{img/results-raw/revision/obs-overhead-with-array.pdf}
\caption{Prefill throughput loss relative to each campaign's baseline (lower is better). (a) Historical P40 values. (b) RTX~5090: ten pairs per original tool and five independent GPU-array pairs (diamonds); ticks show means, whiskers min--max. Symlog axis is linear from $-1$ to $1\%$. The array reduces kernel-return recording overhead to 5.57\% for finite pp512 buffering; its final 23.2~MB readback (10.38~ms) is outside prefill.}
% 相对各批次 baseline 的预填充吞吐损失（越低越好）。(a) 历史 P40 数值。(b) RTX~5090：原工具各十组配对，GPU 数组另有五组独立配对（菱形）；短线表示均值，横线为最小至最大值。对称对数轴在 -1 到 1\% 区间为线性。数组将有限 pp512 缓冲下的内核返回记录开销降至 5.57\%；最终 23.2~MB 回读（10.38~ms）不计入预填充。
