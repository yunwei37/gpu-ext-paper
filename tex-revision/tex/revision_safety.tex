...
% 失败类别与 TCB
We distinguish four failure classes: programs the verifier rejects, resource requests that transition validation turns into no-ops, valid policies that are merely ineffective, and failures outside policy verification entirely (workload OOM, GPU faults, loader or driver bugs), each diagnosed at its own layer.
