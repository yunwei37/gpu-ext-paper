...
\Cref{fig:matched-ports} summarizes the seven matched reimplementations: each panel pairs the original implementation with a \sys{} port of the same policy on the same workload and executor.
% \Cref{fig:matched-ports} 汇总了七个匹配重实现：每个子图在相同工作负载和执行器上，将原始实现与同一策略的 \sys{} 移植版配对。
...
\caption{Matched reimplementations of published policies on RTX~5090: (a)--(g) pair the original implementation with the \sys{} port of the same policy on the same workload and executor; (h) summarizes the measured mechanism cost of running the same policy through \sys{}. Higher is better for token/s, lower for latency and overhead.}
% RTX~5090 上对已发表策略的匹配重实现：(a)--(g) 在相同工作负载和执行器上将原始实现与同一策略的 \sys{} 移植版配对；(h) 汇总通过 \sys{} 执行同一策略的实测机制代价。token/s 越高越好，延迟与开销越低越好。
