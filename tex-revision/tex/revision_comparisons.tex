...
\Cref{fig:matched-ports} summarizes five matched reimplementations (scheduling ports XSched and GPREEMPT appear in \Cref{fig:scheduler-latency}): each panel pairs the original implementation with a \sys{} port of the same policy on the same workload and executor.
% \Cref{fig:matched-ports} 汇总了五个匹配重实现（调度移植版 XSched 和 GPREEMPT 见 \Cref{fig:scheduler-latency}）：每个子图在相同工作负载和执行器上，将原始实现与同一策略的 \sys{} 移植版配对。
