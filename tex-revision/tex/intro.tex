...
We also port policies from seven published resource-management systems, including MoE-Infinity, XSched, and GPREEMPT, through \sys{} and compare them with native implementations (\S\ref{sec:policy-mechanism}).
% 我们还通过 \sys{} 移植了七个已发表资源管理系统的策略，包括 MoE-Infinity、XSched 和 GPREEMPT，并与原生实现比较（\S\ref{sec:policy-mechanism}）。
...
\begin{itemize}[leftmargin=*,itemsep=0pt]
  ...
  \item an evaluation of \sys across four GPU workloads, achieving up to $4.8\times$ throughput improvement and $2\times$ tail latency reduction, with all 59 policies generated by AI agents and no OS kernel panics; comparisons of native and \sys{} implementations of seven published policies isolate the mechanism's cost from the policies' benefits.
  % 在四类 GPU 负载上对 \sys 的评估：吞吐量最高提升 $4.8\times$，尾延迟最多降低 $2\times$，全部 59 个策略由 AI 智能体生成，且无 OS 内核恐慌；对七个已发表策略的原生实现与 \sys{} 实现的比较将机制代价与策略收益分离。
\end{itemize}
