...
We also reproduce policies from seven published resource-management systems, including MoE-Infinity, XSched, and GPREEMPT, through \sys{} and compare them with native implementations (\S\ref{sec:policy-mechanism}).
% 我们还通过 \sys{} 复现了七个已发表资源管理系统的策略，包括 MoE-Infinity、XSched 和 GPREEMPT，并与原生实现比较（\S\ref{sec:policy-mechanism}）。
