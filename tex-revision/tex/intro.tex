...
Finding an optimal GPU resource management policy is challenging.
% 寻找最优的 GPU 资源管理策略颇具挑战。
...
AI-driven exploration offers a promising way to automatically optimize programmable GPU resource management policies, as it can handle the vast, workload-specific policy space that makes manual tuning challenging.
% AI 驱动的探索为自动优化可编程 GPU 资源管理策略提供了有前景的途径，因为它能够应对庞大且随负载而异的策略空间，而这正是手动调优的难点。
...
We derive three requirements: systems should support programmable policy interfaces that are safe, dynamic, and full-stack, so that AI agents can optimize these policies effectively without sacrificing system health.
% 我们提炼出三项要求：系统应支持安全、动态且全栈的可编程策略接口，使 AI 智能体能够在不损害系统健康的前提下有效优化这些策略。
...
We propose \sys, a new system that provides an OS interface for GPU resource management policy that is safe, dynamic, and full-stack.
% 我们提出 \sys，一个提供安全、动态且全栈的 GPU 资源管理策略 OS 接口的新系统。
...
\sys-based policies improve throughput by up to $4.8\times$ over framework offloading and $1.76\times$ over application-tuned baselines, and reduce tail latency by up to $2\times$, with no application modifications or restarts.
% 基于 \sys 的策略相比框架卸载最高提升 $4.8\times$ 吞吐量，相比应用调优的基线最高提升 $1.76\times$，并将尾延迟最多降低 $2\times$，且无需修改或重启应用。
These gains come from the policies, not from an intrinsic BPF speedup; we separately port selected policies from seven published systems (including MoE-Infinity, XSched, and GPREEMPT) and compare them with native implementations on shared executors.
% 这些收益来自策略本身，而非 BPF 的固有加速；我们另行移植七个已发表系统（包括 MoE-Infinity、XSched 和 GPREEMPT）中的选定策略，并在共享执行器上与原生实现比较。
Mechanism costs vary by workload and code path: 0.7\% for Expert Buffering, from a 0.44\% improvement to a 1.18\% cost in POD-Attention operator latency, 3.2\% on a UVM fault path, and mixed read-latency effects in storage-tier admission (\S\ref{sec:policy-mechanism}).
% 机制代价随负载与代码路径变化：Expert Buffering 为 0.7\%，POD-Attention 的算子延迟介于 0.44\% 改善与 1.18\% 成本之间，UVM 缺页路径为 3.2\%，存储层准入的读取延迟效应有好有坏（\S\ref{sec:policy-mechanism}）。
In a historical agent study, AI agents generated all 59 evaluated eBPF policies, including eviction ordering, prefetch strategies, scheduling logic, and device-side work-stealing.
% 在一项历史性的智能体研究中，AI 智能体生成了全部 59 个受测 eBPF 策略（包括逐出顺序、预取策略、调度逻辑和设备端工作窃取）。
No OS kernel panics were observed throughout this process; this is deployment experience, not a proof or guarantee of safety.
% 整个过程中未观察到任何 OS 内核恐慌；这是部署经验，而非对安全性的证明或保证。
