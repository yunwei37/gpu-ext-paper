...
To avoid recompiling applications, \sys allows users to define pipelines to patch kernels at launch with small trampolines at positions such as entry, sampled memory operations, and phase boundaries.
...
consumed by persistent worker blocks. On Blackwell NVIDIA GPUs, we map this to cluster launch control\cite{}, which treats a grid of blocks as cancellable work units and enables true hardware work-stealing style scheduling.
...
subject to the same bounded-execution constraints as other \sys code. For memory, it can attach to memory instructions or other tracepoints, function entry/exit, and issue hints to the host with helper functions. Unlike the eGPU prototype, which used fences and atomic operations, there are no fences or atomic operations in the helpers and attach points of the eBPF extensions.
