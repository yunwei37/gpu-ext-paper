...
gbpf is implemented as a set of extensions to the NVIDIA Open GPU Kernel Modules
and a device-side runtime that executes verified eBPF code inside GPU kernels.
On the host, gbpf adds struct\_ops-style callbacks in the memory management and
scheduling paths and exposes a user-space control-plane API and a gbpf-Device
interface. In the GPU, gbpf integrates a JIT that translates verified eBPF
...
gbpf builds on the NVIDIA Open GPU Kernel Modules (Turing, Ampere, Hopper and
...
On the host, gbpf reuses the standard kernel eBPF verifier to ensure memory
safety, bounded loops, stack limits, and controlled helper and map usage. Where
policies must trigger specific actions in the driver (such as committing a victim
selection or marking a kernel for preemption), we expose small kfuncs that can be
called from eBPF under verifier control.

\subsubsection{gbpf-Device user-space interface}

gbpf-Device provides eBPF-style uprobe and uretprobe hooks for GPU-related
...
At kernel launch time, gbpf optionally JIT-translates these handlers to device
...
When an eBPF section is attached to a device-side hook type, gbpf translates the
verified bytecode into GPU-executable code using a just-in-time compiler. On our
prototype systems we target PTX, which provides a stable virtual ISA and allows
architecture-specific optimization; common eBPF idioms such as map lookups and
updates are lowered to PTX sequences.

To invoke translated code from kernels without recompilation, gbpf inserts small
...
with CUPTI and NVBit; all injected code is confined to the gbpf domain.

\subsubsection{Sandbox and resource bounding}

All device-side gbpf code executes inside a lightweight sandbox. At load time,
...
To execute scalar policy logic efficiently on a SIMT architecture, gbpf aggregates
...
processes its current unit, invokes a device-side gbpf handler (running in one lane)
...
claim or release indices. Progress and behavior probes implemented with gbpf
...
subject to the same sandbox and bounded-execution constraints as other gbpf code.
...
gbpf uses eBPF maps as the unified state abstraction across host and device. We
...
and simplify reasoning about CPU–GPU synchronization for gbpf policies.
