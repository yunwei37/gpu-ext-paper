...
blocks: it identifies a bimodal distribution of iterations per thread; rebalancing
particles based on this information yields an 18\% throughput improvement with
1.8\% tool overhead.
...
a total runtime of 32~ms. Switching to CUDA graphs, guided by \texttt{launchlate},
reduces this to 7.2~ms, with less than 0.5\% overhead. \texttt{mem\_trace}
instruments sampled memory operations and recovers per-warp bank conflict rates
within 2\% of hardware counters, with under 3\% end-to-end overhead across
GEMM, SpMV and stencil kernels (\Cref{fig:obs-overhead}).
...
We now evaluate \sys's memory policy interface on
real applications, focusing on offloading setups where memory placement and
prefetching dominate performance.
