...
We use four unmodified GPU applications to evaluate \sys under realistic
end-to-end workloads. In all cases, \sys attaches via runtime and kernel driver
instrumentation; we do not modify application or framework source code.
...
under \sys and the PyTorch profiler (CUPTI+NVTX). For RQ4, we use PyTorch-based
...
extension and instruments CUDA kernels without changes to
the PyTorch code, models, or training loops.

\paragraph{\texttt{llama.cpp}.}
We use the GPT-OSS-120B configuration in \texttt{llama.cpp} to evaluate expert
offload policies (RQ4). The model is approximately 60~GB on an
RTX~5090 with 32~GB of device memory, so inference runs in oversubscribed mode
with UVM and the framework's built-in offload support. \sys attaches
transparently at the driver level and via device-side handlers that tag expert
regions as ``hot'' or ``cold''. We do not modify \texttt{llama.cpp} itself or
its model implementation.

\paragraph{vLLM.}
We evaluate vLLM on a 30B FP8 MoE model to study KV-cache placement and
offloading policies (RQ4) under realistic serving loads. We use a ShareGPT
...
and GPU handlers. vLLM's source code and serving pipeline are left unchanged.

\paragraph{Faiss.}
We integrate \sys with the GPU backend of Faiss to evaluate vector search with
oversubscribed indices (RQ4). We consider both index build and query workloads
...
centroids and posting lists resident while demoting cold segments. The Faiss
library and application code are not modified.
...
introduced in Section~\ref{sec:eval-app-workloads}, focusing on offloading
setups where memory placement and prefetching dominate performance.

\paragraph{LLM inference: expert and KV-cache offload.}
We next study large-model inference workloads where offloading model state and
KV-cache drives performance.

For a GPT-OSS-120B model (approximately 60~GB) on an RTX~5090 (32~GB), we run
\texttt{llama.cpp} under oversubscribed memory using UVM and its existing framework
offload support as baselines. A simple \sys policy that triggers a max-prefetch
strategy on GPU-side memory events (prefetching additional experts when entering
an expert region and evicting based on expert-level hotness) improves throughput
by up to 5$\times$ compared to default UVM and the framework's own offloading.
This policy is expressed as eBPF callbacks in the driver plus device-side handlers
that mark expert regions as ``will-use soon'' or ``cold'' without modifying the
application.

We then evaluate vLLM on a 30B FP8 MoE model (about 30~GB) on the same GPU.
Without offloading, the model does not fit. We compare vLLM's own CPU-offload
mode (no UVM, \texttt{--cpu-offload-gb 8}) against \sys, which runs vLLM with
UVM and a simple sequential prefetch policy for KV-cache segments. Using the
standard ShareGPT benchmark with 100 requests and around 30 requests in parallel,
\sys improves mean and p99 time-to-first-token by 1.7--2$\times$ and decoding
throughput by about 1.3$\times$ over vLLM's framework-managed offload. Simply
enabling UVM without \sys policies performs worse than vLLM's own offload; the
\sys configuration is roughly 2--3$\times$ faster than this UVM-only baseline.

\paragraph{Vector search and GNN training.}
We integrate \sys with Faiss for vector search and with PyTorch for GNN training,
using UVM-based oversubscription.

For Faiss, we evaluate index build and query latency when the index does not fit
in device memory. A \sys policy that adaptively prefetches index segments based
on observed PCIe traffic and access streams reduces index build time by about
1.5$\times$ compared to default UVM behavior. Query workloads see similar
benefits from keeping frequently accessed centroids and posting lists resident
and demoting cold segments.
...
\subsection{RQ5: Fine-grain Scheduling}
