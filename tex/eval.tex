...
GPU. Server~B is a dual-socket Intel Gold 6138 processor (80 cores, 2.0~GHz, 55~MB LLC) equipped with 256~GB DDR3 RAM and an NVIDIA P40 GPU. Unless otherwise specified, we evaluate the experiment on machine A, average each metric over 10 trials; when averaging speedups we use geometric mean. We compare against default UVM
...
\caption{Impact of \sys block-scheduling policies across workload regimes. (a) Under moderate imbalance (GEMM), simple always-steal policies reduce latency by $\sim$11\% and behave essentially identically. (b) Under clustered heavy tails, Greedy becomes pathological (+20.3\% latency) while LatencyBudget matches baseline performance, demonstrating the need for workload-aware policy selection.}
...
\begin{figure}
\centeringOn imbalanced GEMM with variable MxNxK configurations, TokenBucket achieve
...
We evaluate \sys's BPF struct\_ops scheduling policies on \emph{compute-bound} multi-tenant workloads where the bottleneck is GPU SM time rather than memory bandwidth. In this scenario, 2 latency-critical (LC) processes and 4 best-effort (BE) processes run concurrently, each with 4 CUDA streams submitting 50 compute kernels ($\sim$80ms/kernel). We compare the native scheduler against \sys with differentiated timeslices (LC: 1s, BE: 200$\mu$s) using a gpreempt-style preemption control policy, collecting 12,000 kernel launch latency samples over 10 rounds per configuration. \Cref{fig:scheduler-latency} shows that \sys reduces the per-round P99 launch latency mean for LC workloads from 1188$\mu$s to 53$\mu$s (95.5\% reduction), while P99 standard deviation drops from 3405$\mu$s to 35$\mu$s (99\% reduction), demonstrating significantly improved scheduling stability. BE throughput remains essentially unchanged (+0.8\%), confirming that \sys's BPF struct\_ops can dramatically reduce LC tail latency without sacrificing BE throughput.
...
\subsubsection{Host Runtime Oberhead}
