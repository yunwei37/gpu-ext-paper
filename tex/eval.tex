...
via \texttt{should\_try\_steal}, we evaluate on a minimal LLM inference framework
and graph algorithms where static block assignment leads to some blocks being
overloaded while others idle.

% TODO: Add actual experiment data
% Workload: minimal LLM or graph algorithm with load imbalance
% Compare: static block assignment vs gBPF device-side block scheduler
% Expected: ~10% improvement in throughput
\begin{figure}
\centering
\fbox{\parbox{\columnwidth}{\centering [Figure: Single-Tenant Block Scheduler]}}
\caption{Throughput improvement from \sys device-side block scheduler with work stealing vs static block assignment on load-imbalanced workloads (TODO: +X\% improvement).}
...
We evaluate \sys's scheduling interface using priority-based policies on mixed multi-tenant workloads that combine high-priority latency-sensitive kernels with low-priority batch jobs. We focus on tail latency, throughput, and fairness.
...
TODO: Policy complexity and generality (LOC and overhead). Table showing Lines-of-Code required to implement different memory/scheduling policies across PyTorch, llama.cpp, vLLM, and Faiss workloads. Policies are concise (typically <200 lines eBPF code). Figure plotting overhead vs. policy complexity (helper calls, map accesses). Even complex tenant-aware policies incur less than 5\% overhead.
