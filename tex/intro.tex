...
% 1. (General problem) GPU is important (1). Motivate the resources management policy is important (resource mpolicy has emerged as ... 
GPUs dominate the compute and capital costs of modern systems~\cite{jeon2019analysis}, powering diverse workloads ranging from latency-sensitive large language model inference pipelines to large-scale distributed training~\cite{kwon2023efficient,shoeybi2019megatron}, graph analytics~\cite{lin2024towards,chen2020pangolin,wang2016gunrock}, vector search~\cite{pan2024survey,cao2023gpu}, and data visualization~\cite{moreland2016vtk}. Effective system performance and efficiency across these workloads increasingly depend not just on raw compute power but critically on adaptive resource management \emph{policies}. Such policies orchestrate data placement and migration between host and device memory~\cite{sun2024llumnix,zhong2024distserve,patel2024splitwise,strati2024orion}, manage kernel admission, scheduling, and preemption in multi-tenant deployments~\cite{fu2024serverlessllm,shubha2024usher,hu2023lucid,li2022miso,jayaram2023sia}, and enable dynamic observability for online adaptation and optimization~\cite{agrawal2024vidur}. Crucially, these policies must continuously evolve to adapt to changing hardware architectures, emerging workload characteristics, and varying service-level agreements (SLAs)~\cite{agrawal2024taming,yu2024orca,kwon2023efficient,xiao2018gandiva,yu2020fine,sheng2023flexgen,rasley2020deepspeed,delimitrou2013paragon,delimitrou2014quasar,lo2015heracles,qiao2021pollux,narayanan2020heterogeneity}.
...
% 2. (The problem with current works) Current design rigiet for us to create new resource management policy, some one has try to modify drivers; others are trying to do it in usersapce framwork
% Have one sentence to introduce the GPU stack, includeusersapce framework, use runtime, driver, firmware, device code.
Despite their growing importance, GPU resource management policies today remain constrained by rigid system abstractions. On one hand, vendor GPU drivers and firmware enforce static, opaque mechanisms lacking programmability and adaptability~\cite{nvidia-open-gpu}. \yusheng{this make the performance bad... }
While academic works like TimeGraph and Gdev have shifted GPU scheduling and memory control into Linux device drivers\cite{kato2011timegraph,kato2012gdev}\yusheng{to improve performance...}, and recent systems further refactor driver-level control paths\cite{kernelintercept,gpreempt,wang2024gcaps}, these efforts typically implement policies inside static kernel modules. Lacking a safe, programmable interface, adapting policies requires invasive driver changes and operational disruptions.  To enable flexibility without kernel modification, recent serving systems retrofit programmability above the driver in the user-space.
...
Some profiling-focused instrumentation frameworks\cite{cupti,villa2019nvbit,neutrino,egpu}, also allow instrucmented the GPU kernels for profiling only, but remain isolated from the host side.
...
Thus, designing a general GPU policy substrate introduces three challenges.

\xiangyu{I have a feeling that we can use ``goals" instead of challenges. We can say sth. such as Thus, designing a general GPU policy substrate should satisfy three goals.}
