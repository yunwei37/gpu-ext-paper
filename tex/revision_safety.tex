...
A 15-cell CPU loader matrix exercises \texttt{LD\_PRELOAD}$\rightarrow$\texttt{BPF\_PROG\_LOAD}. In three repetitions, strict mode rejects an out-of-stack write without consuming a program ID; warning, no-verification, and default modes admit it. This validates loader configuration, not GPU or driver safety.

Two fresh-process strict-mode pairs on RTX 5090 and the verifier-enabled Table~1
runtime validate the real counter path: each admitted 13-instruction callback
