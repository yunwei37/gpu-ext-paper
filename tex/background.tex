...
an OS component, but it still works in userspace as a library~\cite{lithos}.
...
resource management. 
First, their programmability is \emph{framework-scoped}:
...
stack. They typically require modifying the application to integrate them.
Second, they operate strictly above vendor drivers and firmware and thus
...
preventing dynamic sharing with other processes, and require additional frameworks to handle that~\cite{lmcache}. Similarly,
hardware partitioning techniques like MIG enforce fixed boundaries that cannot
adapt to bursty traffic.
Third, user-space schedulers are confined to
...
Fourth, even within a single process, they are unprivileged:
...
Moreover, they cannot work on latency-critical paths such as page fault handlers.
Also, tools like XSched cannot express policies that depend on warp-level behavior (e.g., ``only preempt when most blocks have passed the barrier in phase 2'') or GPU-side access patterns feeding into a driver-level region cache. Typically they require additional compiler integration to do it, making them non-transparent.
...
bug crashes or hangs the application), and typically operate in a read-only manner, unable to actively regulate system behavior such as memory placement or thread scheduling. Also, they lack privileged driver integration since many controls happen on the CPU, so they cannot do control beyond a single kernel. Using assembly directly like Neutrino also limits its expressiveness. Recent user-space approaches like the eGPU workshop paper~\cite{egpu} demo extend eBPF to GPUs for observability, limiting them to application-level instrumentation and incurring prohibitive overheads (up to $1000\times$, see \S\ref{sec:eval}).
