...
% BPF + maps
\node[bpf, minimum height=1.8cm, minimum width=1.4cm, text width=1.2cm, align=center] (g_bpf) at (-0.5, -4.0) {BPF\\+ maps};
...
\end{tikzpicture}
\caption{Execution model contrast. sched\_ext: one synchronous callback per transition (1:1). \sys{}: triggers from any stack layer direct multiple asynchronous lifecycle transitions via kfunc effects (many:many). The dashed line marks the host-device boundary.}
