...
use both to report observed accesses and to express future-use hints. At selected
memory operation sites, \sys injects lightweight hooks that operate at warp
granularity and invoke a device-side \sys handler with an \emph{ACCESS} event:
...
that can be dynamically assigned to \emph{worker} thread blocks. Policies interact
with this internal scheduler through two mechanisms:
