...
Figure~\ref{fig:system-design} illustrates the high-level architecture of \sys, consisting of a user-space control plane and a unified kernel-device execution path. A \sys policy is structured as a distributed application comprising host- and device-side eBPF programs, hierarchical cross-layer maps, and a user-space control-plane for lifecycle management and dynamic reconfiguration.

\paragraph{User-space Control Plane.}
Policy authors write \sys policies using standard eBPF tooling (e.g., \texttt{clang/libbpf}, \texttt{bpftool}, \texttt{bpftrace}). The control-plane application uses a \sys loader library to compile policies into eBPF bytecode, create hierarchical maps, and load policies into kernel and device execution contexts. It manages policy lifecycle operations (installing, updating, removing), enables dynamic reconfiguration (e.g., adjusting eviction thresholds, sampling rates, or tenant priorities), and exports aggregated runtime metrics (e.g., per-region hotness, tenant latencies) to external monitoring or adaptation systems without application or kernel restarts.

\paragraph{Kernel and Device Execution Path.}
\sys provides a unified runtime spanning OS kernel and GPU device contexts that manages verification, compilation, cross-layer state management, and policy execution. Upon policy load requests, the runtime invokes a SIMT-aware static verifier (\S\ref{sec:verification}) extending standard eBPF verification to enforce warp-uniform semantics, bounded execution, memory safety, and GPU-specific synchronization constraints. Verified programs are JIT-compiled into host-native instructions for kernel hooks and GPU-compatible code (e.g., PTX or SPIR-V) for device hooks.

This runtime transparently manages hierarchical eBPF maps, exposing a stable logical abstraction and key space to policy programs while internally optimizing placement, sharding, and synchronization across host memory, GPU global memory, and GPU on-chip memory tiers. Host- and device-side handlers access shared state exclusively via standard eBPF helper interfaces, abstracting away manual synchronization and cross-layer address translation.

Verified eBPF handlers execute at structured hook points defined in GPU drivers and GPU kernels. On the host side, \sysdriver exposes \texttt{struct_ops}-style hooks for memory placement and kernel scheduling decisions (e.g., region admission/eviction, kernel submission/admission). When invoked, host-side handlers process strongly-typed BTF contexts and return concise policy decisions (\emph{admit}, \emph{evict}, scheduling priorities), which \sysdriver enforces via safe kernel helpers.

On the GPU device side, \sys injects lightweight trampolines at kernel entry/exit points, warp-synchronization barriers, and critical memory operations. Trampolines invoke device-side handlers at warp granularity, enabling handlers to perform fine-grained policy decisions (e.g., dynamically prefetch regions, pin frequently accessed data, or adjust block-level scheduling priorities). Device-side handlers also update cross-layer shared maps with detailed performance signals (per-region access frequency, warp occupancy), but do not directly manipulate hardware control registers or page tables.

Together, the control plane, unified runtime, and structured hook execution form a tightly integrated, adaptive feedback loop: device-side logic directly implements granular GPU resource control and updates fine-grained instrumentation state; host-side handlers utilize this state to enforce global placement and scheduling decisions; and the control-plane continuously monitors policy effectiveness, dynamically adapting configurations or loading new policies as needed.
...
struct gdrv_mem_ops {
  ...
  int (*region_add)(struct gdrv_mem_region_add_ctx *ctx);
  // Called when memory subsystem observes region use; decide cache hit,
  int (*region_access)(struct gdrv_mem_region_access_ctx *ctx);
  // Called when a region is about to leave the device-resident working set.
  int (*region_remove)(struct gdrv_mem_region_remove_ctx *ctx);
