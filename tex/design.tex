...
To ensure OS kernel safety boundaries, in \sys{}, verified kfuncs structure lifecycle transitions explicitly only along three dimensions: \emph{ordering} (evict chunk A before B), \emph{timing} (prefetch now rather than waiting), and \emph{selection} (preempt tenant X for Y), and the lifecycle is always managed by the driver mechanism. For example, policies only implement transitions to reorder driver-maintained eviction lists, does not allow allocating memory and will fall back to default FIFO behavior under pressure or misconfiguration. In multi-tenant settings, a single administrator-defined policy governs all tenants, as with sched\_ext; per-tenant policy isolation is future work.
