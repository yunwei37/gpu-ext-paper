...
\subsection{Safety Model and Optimization}
\label{sec:safety}
...
execute within strict runtime bounds when running on the GPU. On the host, \sys
reuses the standard eBPF verifier to ensure that programs are memory-safe, free of
unbounded loops, and respect limits on stack depth and helper usage. The verifier
only allows access to approved helper functions and supported map types, and rejects
programs that could corrupt driver state or violate kernel invariants. For device-side
execution, \sys applies the same verifier to the eBPF bytecode before any translation
to PTX, so the control-flow and memory-safety properties proven by the verifier hold
for both host and device instantiations of a policy.

After verification, device-side policy code is translated to PTX and executed in a
sandboxed GPU runtime. The sandbox bounds the number of instructions per invocation,
constrains register allocation to a reserved set so as not to clobber application
...
minimal live register and predicate state around calls into translated \sys code.
Taken together, static verification and the sandbox guarantee that injected policy
logic is bounded, deterministic, and cannot hang the GPU, corrupt kernel or application
state, or impose unbounded latency on performance-critical paths such as memory fault
handling or kernel admission.

Safety is also a prerequisite for efficiency: by proving properties once in the
verifier and enforcing strict isolation in the sandbox, \sys can aggressively optimize
device-side execution without weakening its guarantees. The PTX JIT leverages
verifier results to remove redundant bounds checks and zeroing, specialize programs
per hook type, and map common eBPF idioms (such as map lookup-and-increment or
``lookup-or-initialize'' patterns) to short, GPU-friendly instruction sequences.
\sys further adopts a warp-centric execution model for device-side policies: when a
device hook fires, a single designated lane per warp typically runs the scalar
\sys snippet and updates shared maps on behalf of the warp, while other lanes
provide their local inputs via warp-synchronous operations or remain idle. This
approach preserves the simple scalar programming model of eBPF, but reduces the
number of instructions executed for policy logic by up to a warp factor and avoids
warp divergence inside policy code.

Finally, \sys optimizes map access and instrumentation overhead to respect GPU memory
hierarchy and contention patterns. For simple counters and histograms, the runtime
aggregates updates in warp-local or SM-local accumulators in registers or shared
memory and periodically flushes them to global eBPF maps, preventing global atomics while preserving logical map semantics. Context structures
passed to device-side policies are minimized and packed on a per-program basis,
materializing only the fields that the verifier proves may be read and precomputing
derived identifiers in the trampoline. For observability-heavy policies, trampolines
also support configurable sampling: a per-warp counter and a sampling parameter
stored in a control map allow \sys to invoke device-side policy code only every
$N$-th event. These optimizations, combined with the safety model above, ensure that
\sys can execute policy code directly on GPU hot paths with low, predictable overhead.
