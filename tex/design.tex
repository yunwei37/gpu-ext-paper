...
\sys policies are expressed as eBPF programs using a unified intermediate representation: a restricted subset of eBPF instructions and helpers. All \sys policies are first compiled to standard eBPF bytecode with existing toolchains (clang + libbpf) and loaded into the kernel using unmodified eBPF system calls. The kernel’s eBPF verifier remains the only verifier: it enforces bounded loops, stack limits, and memory safety, and it checks that each program uses only the \sys-specific kfuncs and hook contexts declared via BTF annotations. We do not implement a separate user-space verifier.
...
A fundamental challenge of programmable GPU infrastructure is safely executing untrusted policy logic within latency-critical paths, given the absence of traditional OS-level isolation and the potential for severe consequences from faulty code (e.g., GPU hangs or memory corruption). To address this challenge, \sys adopts a unified safety model grounded in static verification and rigorously bounded runtime execution. Policies are expressed in a restricted subset of eBPF, verified statically to guarantee memory safety, bounded loops, deterministic termination, and controlled resource usage. At runtime, a sandboxed environment strictly enforces execution budgets and memory access constraints, preventing injected policy logic from interfering with GPU kernels, corrupting driver state, or causing unpredictable latency spikes. This approach provides strong isolation guarantees, enabling confident deployment of complex adaptive policies directly into sensitive GPU driver and kernel paths.

For GPU-side execution, \sys does not re-verify policies. Instead, it treats the device code compiler (e.g., to PTX or SPIR-V) as a trusted translation of already-verified eBPF bytecode into device code. The GPU sandbox enforces an instruction bound derived from the verifier’s instruction-count budget and restricts memory accesses to addresses reachable via BPF map helpers; it cannot grant additional capabilities beyond those approved by the kernel verifier.

\paragraph{Optimizing for GPU's SIMT Execution.}
A key difficulty in deploying policy logic on GPUs is efficiently executing scalar decision-making code within the massively parallel Single Instruction Multiple Thread (SIMT) architecture. Naively executing scalar control logic directly across all GPU threads introduces prohibitive divergence and overhead, severely degrading GPU efficiency. To overcome this, \sys explicitly designs policy execution around SIMT execution characteristics: it aggregates scalar policy logic execution at warp granularity, typically executing decisions on a designated warp lane while efficiently synchronizing minimal input from other lanes. Frequent global state updates—common in observability and adaptive policy logic—are optimized through warp-level and SM-level local aggregation techniques, dramatically reducing contention and costly global memory operations. Moreover, context metadata passed to policy handlers is minimized through static analysis, passing only essential fields to reduce memory traffic and latency. Finally, adaptive sampling mechanisms allow policies to dynamically tune instrumentation frequency, achieving low overhead across diverse workloads. Collectively, these optimizations enable efficient, scalable, and minimally invasive GPU-side policy execution suitable for production deployment.
...
// Example removal context with an eviction candidate set.
struct memsys_region_remove_ctx {
  __u32 tenant_id;
  __u32 cause;        /* lifetime vs pressure */
  __u32 nr_candidates;
  __u64 candidates[MEMSYS_MAX_EVICT_CANDIDATES];  /* region_ids */
};
\end{verbatim}
...
use both to report observed accesses and to express future-use hints. A compact
device context is passed to each handler:

\begin{verbatim}
struct gpu_dev_mem_ctx {
  __u64 region_id;   /* logical region (weights, KV shard, etc.) */
  __u32 kernel_id;
  __u32 warp_id;
  __u32 sm_id;
  __u32 event;       /* ACCESS or HINT */
  __u32 hint_flags;  /* e.g., WILL_USE_SOON, COLD, ... */
};
\end{verbatim}
...
the handler receives metadata such as a logical region identifier, warp and SM
identifiers, and a kernel or phase identifier, and updates shared maps with
per-region statistics such as access counts, approximate working-set membership,
and simple locality signatures. To bound overhead, \sys supports sampling (for
example, only one in every $N$ accesses generates an event) and per-warp aggregation
before these statistics are written to shared maps.
...
observe and steer this internal scheduler via a compact per-block context:

\begin{verbatim}
struct gpu_block_sched_ctx {
  __u32 kernel_id;
  __u32 tenant_id;
  __u32 cluster_id;
  __u32 sm_id;

  __u32 prev_work_id;   /* last completed work unit (tile index) */
  __u32 next_work_id;   /* chosen work unit, written by policy */
};
\end{verbatim}
...
\begin{itemize}[leftmargin=*]
  \item \emph{Block scheduling callbacks}, where a worker block periodically invokes
        a device-side \sys handler to select its next work unit. The context includes
        the kernel or phase identifier, tenant information, and local progress
        metrics. The handler consults shared maps and returns a work descriptor
        (for example, a tile index or logical block identifier) or an indication
        that no further work is available. This enables policies to implement
        priority-aware or tenant-aware work stealing and load balancing at the
        thread-block level.
...
policies can consume at the next host-side scheduling hook. Together, the host-side
and device-side interfaces allow \sys policies to coordinate decisions across the
full scheduling pipeline: from kernel admission and preemption on the host to
fine-grained block-level work distribution inside GPU kernels.
