...
GPU resource management has to run at the driver layer to be effective, but GPU drivers and firmware were never designed to expose a programmable interface. Naively surfacing low-level mechanisms (page tables, command buffers, interrupt handlers) to eBPF would give policies a lot of power but make the driver brittle, version‑dependent, and unsafe in multi‑tenant settings. The challenge is to carve out a narrow, stable, and verifiable GPU policy interface—rich enough to express real memory and scheduling policies, yet constrained enough that misbehaving policies cannot corrupt device state or crash the kernel.

\subsubsection{C2: Mismatch Between the single thread CPU model and the SIMT GPU Execution Model}
...
Even if GPU drivers exposed extensible interfaces, directly applying existing programmable CPU-oriented eBPF mechanisms does not work because of the conflict between the single thread eBPF and the GPU Single Instruction Multiple Threads (SIMT) execution model. 
