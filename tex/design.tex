...
Figure~\ref{fig:system-design} shows a unified user-space eBPF runtime that keeps verification, JIT, and maps in user space by default. Programs compiled with \texttt{clang}/\texttt{libbpf} write to maps in a shared-memory region (e.g., \texttt{boost::managed\_shared\_memory}) visible to CPU threads and GPU kernels. On demand, eBPF bytecode is translated to PTX and injected at GPU hook points so device-side probes write into the same maps, yielding real-time, low-overhead cross CPU–GPU observability.

\begin{figure}[ht]
  \centering
  \fbox{\parbox{.82\linewidth}{\centering [Figure: \sys Gate Design]}}
  \vspace{-.5ex}
...
In this section, we will talk about the \sys implementation.
\label{sec:design}
...
On non-coherent systems, cross-domain visibility requires explicit barriers (device \_\_threadfence\_system and host stream sync). We encapsulate these as a simple device/host pair to centralize tuning and reduce boilerplate:
\begin{minted}[highlightlines=false,fontsize=\small]{c}
...
\end{minted}
In a persistent kernel, call the GPU fence before reading shared state; after host updates, synchronize the stream to ensure visibility. This two-call barrier hides cross-domain complexity.
\begin{minted}[highlightlines=false,fontsize=\small]{c}
...
Dynamic PTX injection must not corrupt application state. \sys therefore runs injected snippets in a protected domain through eBPF verification and runtime sandboxing.
...
%\begin{itemize}[leftmargin=*,itemsep=0pt]
  ...
  %\item
  b) Executes the verified instrumentation logic within bounded memory regions.
  %\item
  c) Restores registers and resumes normal kernel execution.
%\end{itemize}

The eBPF verifier ensures injected code operates only on designated shared memory regions, preventing corruption of application state. Combined with trampolines and shared-memory coordination, this approach ensures injected code remains isolated from application kernels.
