...
\sys only executes policy code that is expressed in a restricted eBPF subset and accepted by the in-kernel verifier, each type with certain kfunc. It's much more important and challenge on GPU that we need to aware the SIMT model and the multi-layer nature of memory, for both efficiency and Safety, since the eBPF bytecode and verifier is not design for it. This design intentionally trades policy expressiveness for strong guarantees and efficiency: policy handlers cannot hang the GPU, corrupt driver or application memory, introduce unbounded latency on critical paths or use too much resource that hurt the application.
...
The CPU‑side verifier and bytecode under a model where
Execution is single‑threaded, under high frequency CPU, can efficiency handle multiple branch and memory access, Memory accesses go through helpers/maps with specific semantics.

simply reusing eBPF verifier and JIT compiler infrastructure on GPU is challenge: it does not aware Concurrency and cross‑domain memory semantics...
The Execution is SIMT, with many warps in parallel.The memory model (UM, CUDA, __threadfence_system) is different.You’re re‑implementing helpers (map access) yourself in PTX. Concurrent access patterns (thousands of warps + CPU) are far more intense.
...
So, we explictly extend kernel eBPF verifier through kernel module to:
1. use conservative work budgets for execution instructions, memory useage, and memory access count per instructions, to avoid memory bandwidth overlord. Define per-program-type budgets that reflect how often hooks fire and what overhead you can tolerate, e.g. insn_bound ≤ B_max_prog_type, map_ops ≤ M_max_prog_type, branch < M_max_prog_type
...
%         set. The context includes a cause flag and a candidate set.
% \end{itemize}

% In addition, \sys provides a \emph{prefetch hook} that runs at safe points where
% data can be moved proactively, for example between kernel launches or at page fault. 
% The prefetch context includes a snapshot of current region state,
% available capacity, and the policy returns a ranked
% list of regions to promote to device memory. 
