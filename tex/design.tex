...
GPU SIMT execution imposes constraints that traditional scalar eBPF does not address. Executing an eBPF handler independently in each GPU lane would cause severe warp divergence, unpredictable resource usage, and unsafe side effects. \sys resolves this mismatch by introducing a SIMT-aware verification model that ensures all GPU-side policy execution is \emph{warp-coherent}. The key insight is the distinction between \emph{warp-uniform} and \emph{lane-varying} values. Warp-uniform values such as region identifiers, kernel identifiers, static configuration parameters, and tenant identifiers, are identical across all threads in a warp. In contrast, lane-varying values arise from per-thread GPU execution state, including effective addresses, thread indices, and lane identifiers. Policies may freely compute with lane-varying values, but lane-varying quantities may not influence control flow or directly produce observable side effects.

To guarantee this discipline, \sys requires that all branch conditions, loop bounds, and map-update keys must be warp-uniform. Map updates and scheduling or memory hints must be expressible as warp-level decisions: their keys must be uniform, and their values must be derived from uniform quantities or warp-level reductions of lane-local contributions. This design eliminates divergence, bounds per-warp resource usage, and ensures that device-side policy execution is compatible with GPU hardware semantics.

\sys further enforces safety properties aligned with the GPU execution model: policies may not issue GPU-wide barriers, device-level synchronization primitives, or non-uniform atomic operations. Each hook type carries an explicit resource budget, bounding the number of eBPF instructions, helper calls, and memory operations permitted within a policy handler. These constraints, combined with the SIMT-aware uniformity rules, ensure that programmable control-plane logic can safely run inside privileged GPU driver and kernel contexts.
