...
GPU resource management needs to run at the driver layer to be effective, but GPU drivers were never designed to expose a programmable interface. Exposing low-level mechanisms (page tables, command buffers, interrupt handlers) to eBPF would give policies maximum expressiveness but compromise driver stability and tenant isolation, risking kernel crashes. Conversely, constraining programmability to high-level abstractions ensures safety but severely limits the expressiveness needed for complex memory placement and scheduling decisions. Thus, the challenge lies in carving out a narrow, stable, and verifiable GPU policy interface: expressive enough to implement real-world policies, yet constrained enough to prevent misbehaving policies from corrupting device state or crashing the kernel.

\subsubsection{C2: Mismatch Between CPU Extension Semantics and the GPU SIMT Execution Model}
Applying existing CPU-oriented eBPF mechanisms to GPU device is difficult due to semantic mismatches with the GPU SIMT execution model. CPU-side eBPF assumes single-threaded execution, whereas GPU kernels execute in warps of 32 threads that must follow warp-uniform control paths. Running scalar eBPF logic per thread introduces warp divergence, serialization overhead, and dead lock risks from thousands of concurrent memory operations unchecked by the CPU-side verifier. Furthermore, GPUs lack isolation, recovery points meaning an unbounded loop or invalid access in policy logic can halt the entire device. This semantic mismatch makes reuse of CPU-oriented eBPF unsafe and slow.
