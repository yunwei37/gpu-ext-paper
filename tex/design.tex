...
For GPU-side execution, \sys does not re-verify policies. Instead, it treats the PTX compiler as a trusted translation of already-verified eBPF bytecode into device code. The GPU sandbox enforces an instruction bound derived from the verifier’s instruction-count budget and restricts memory accesses to addresses reachable via BPF map helpers; it cannot grant additional capabilities beyond those approved by the kernel verifier.
