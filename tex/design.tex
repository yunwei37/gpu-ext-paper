...
We define explicit, relaxed consistency semantics using bounded freshness epochs: map updates from GPU-side handlers become visible to host-side handlers within bounded time intervals, sufficient for policy purposes such as eviction or scheduling decisions. This abstraction allows developers to express complex cross-layer logic (e.g., GPU-side region access tracking informing host-side eviction policies) using simple eBPF APIs without explicit synchronization overhead.
