...
GPU drivers, such as NVIDIA's open GPU kernel modules, are large, monolithic codebases tightly integrating low-level hardware mechanisms and resource-management policies. While they expose APIs for applications to provide hints (e.g., \texttt{cudaMemAdvise}) or adjust parameters, these interfaces only allow applications to influence fixed built-in policies; they do not support defining custom algorithms such as workload-specific eviction or tenant-aware scheduling. Unlike Linux kernel subsystems explicitly designed with stable eBPF extension points (e.g., \texttt{struct\_ops}), GPU drivers lack interfaces designed for policy programmability. \textbf{Policy logic embedded in driver mechanisms} complicates the introduction of attach points that remain stable across driver versions and GPU generations. For instance, in NVIDIA's Unified Virtual Memory (UVM) kernel module, the page-fault handler directly embeds LRU-based eviction and tree-based prefetch policies, making it difficult to interpose on memory placement without modifying the driver code path. Moreover, since policy logic executes directly in privileged driver contexts with tight latency constraints and direct GPU hardware control, there are \textbf{strict driver safety requirements}: logic errors (e.g., incorrect lock usage or inconsistent command submission) can cause device-wide stalls or node-level failures, mandating bounded execution and memory safety guarantees.
...
two driver-level types for GPU policy hooks (\texttt{BPF\_PROG\_TYPE\_GPU\_MEM} for memory events and \texttt{BPF\_PROG\_TYPE\_GPU\_SCHED} for scheduling) and one device-level type (\texttt{BPF\_PROG\_TYPE\_GPU\_DEV}) for GPU-kernel instrumentation. These types coexist with existing kernel eBPF program types (e.g., cgroup, TC) without conflict; they reuse the same verifier, loader, and map abstractions, and are distinguished only by their attach points and SIMT-specific verifier rules.
...
// Host-side kfuncs: reorder regions in kernel-owned eviction list
...
struct gdev_mem_ops {
    // Warp observed memory access; update tracking maps
    void (*access)(gdev_mem_access_ctx_t *ctx);
    // Kernel provides future-use hint for prefetch/pin
    void (*hint)(gdev_mem_hint_ctx_t *ctx);
};
// Device-side helpers
...
such as \emph{admit}, \emph{bypass}, \emph{pin}, or \emph{evict}. The underlying memory subsystem enforces these decisions
...
handlers receive a compact, warp-uniform context struct with fields such as a logical
...
warp granularity and invoke the device-side \texttt{access} handler with an \emph{ACCESS} event.
Each handler is verified to be warp-uniform and executes via a single warp-leader lane;
other lanes remain idle at the hook, avoiding divergence and per-lane serialization (\S\ref{sec:verification}).
The handler updates hierarchical maps with per-region statistics such as access counts,
...
demotion. Both ACCESS and HINT events are recorded in hierarchical maps and per-SM control
queues that the host periodically drains; host-side handlers consult this state at \texttt{region\_access}, \texttt{region\_remove}, and \texttt{prefetch} hooks to implement placement, prefetch, and eviction decisions.
...
\emph{which kernels reach the GPU} (kernel granularity) and \emph{how work is consumed inside those kernels} (thread-block granularity), enabling fine-grained priority, deadline-aware, and power-aware scheduling while respecting the GPU SIMT execution model.
...
On the host, \sys extends the GPU kernel lifecycle with attach points for admission, ordering, and preemption of kernels. The \texttt{submit} handler decides whether to enqueue a kernel (return 0) or reject/defer it (return $<$0); the \texttt{admit} handler selects the next kernel ID to dispatch or returns $-1$ to leave the GPU idle; and the \texttt{complete} handler runs after kernel completion for bookkeeping.

Attached eBPF schedulers read these contexts and hierarchical maps (for example, per-tenant
latency and throughput statistics, outstanding work across queues) and return compact
decisions such as which kernel to run next or whether to throttle a tenant. Policies can also request preemption of a running kernel via the \texttt{gdrv\_sched\_preempt} kfunc, which triggers a best-effort cooperative preemption at predefined safe points within the kernel. The host-side interface thus controls which kernels occupy GPU contexts and how they are multiplexed in multi-tenant scenarios.
